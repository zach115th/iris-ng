...
def configure_module(args: argparse.Namespace, api_key: str) -> dict[str, Any]:
    ...
    # Strip config blobs — both contain the MISP API key and AI backend key in plaintext.
    refreshed_safe = {k: v for k, v in refreshed.items()
                      if k not in ("module_configuration", "module_config")}

    return {
        "module_id": module_id,
        "import": imported,
        "enable": enabled,
        "module": refreshed_safe,
        "resolved_defaults": {
            "misp_url": misp_env["url"],
            "verify_tls": verify_tls,
            "org_id": org_id,
