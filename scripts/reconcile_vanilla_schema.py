...
def _drop_legacy_ioc_link():
    """Drop the legacy `ioc_link` M2M table after backfill.

    Upstream migration 3715d4fac4de migrated `ioc_link` rows into
    `ioc.case_id` but left `op.drop_table('ioc_link')` commented out
    (see line 343 of that migration). The table sits around with its
    FK ioc_link.ioc_id -> ioc.ioc_id intact, which then blocks any
    case-delete because delete_case() does `Ioc.query.delete()` and
    Postgres refuses on FK violation. No iris-ng code reads `ioc_link`
    anymore, so dropping it after backfill is safe and removes the
    only known cause of "delete case silently fails" on vanilla
    imports."""
    inspector = inspect(db.engine)
    if 'ioc_link' not in set(inspector.get_table_names()):
        return False
    db.session.execute(text('DROP TABLE IF EXISTS "ioc_link" CASCADE'))
    db.session.commit()
    print("  + ioc_link table dropped (legacy M2M, unblocks case delete)")
    return True
...
def reconcile():
    with app.app_context():
        ...
        try:
            if not _drop_legacy_ioc_link():
                print("  ioc_link: not present (fresh iris-ng schema)")
        except Exception as exc:
            print(f"  ! ioc_link drop FAILED: {exc}", file=sys.stderr)
            db.session.rollback()
            failed.append(("ioc_link", "drop", str(exc)))
