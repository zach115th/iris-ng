...
EXPECTED_TABLES=("cases" "alerts" "ioc" "case_assets" "cases_events"
...
IRIS_NG_NEW_TABLES=("case_ai_artifact" "case_working_event" "ioc_note_link"
                    ...
                    "skill" "user_skill" "case_analyst_link" "case_required_skill"
                    "case_time_entry" "evidence_drive" "ai_job")
...
import_run() {
    ...
    # 1. Stop app + worker + ai_worker (keep db up — we need it to restore into).
    log ""
    log "[1/8] Stopping app + worker + ai_worker (db stays up)..."
    docker compose -f "$IRIS_NG_COMPOSE" stop app worker ai_worker || warn "app/worker/ai_worker were not running"

    # 2. Ensure db is up.
    log "[2/8] Ensuring db is up..."
    ...
    #    image is postgres:17-alpine (and the user may have an older Postgres
    #    on their old vanilla host). Manually terminate open backends first,
    #    then plain DROP — works on Pg >= 9.6.
    log "[3/8] Restoring Postgres dump (drop + recreate iris_db)..."
    ...
    log "[4/8] Pre-Alembic schema sanity check..."
    local missing=()
    ...
    if [[ "$skip_volumes" -eq 0 ]]; then
        log "[5/8] Restoring named volumes..."
        for vol_base in "${NAMED_VOLUMES[@]}"; do
            local arc="${from_dir}/${vol_base}.tar.gz"
            local vol="${project_name}_${vol_base}"
            if [[ ! -f "$arc" ]]; then
        ...
        log "[5/8] --skip-volumes: not touching named volumes"
    fi
    ...
    if [[ "$skip_secrets" -eq 0 ]] && [[ -f "${from_dir}/secrets.env" ]]; then
        log "[6/8] Carrying IRIS_SECRET_KEY + IRIS_SECURITY_PASSWORD_SALT into iris-ng .env..."
        if [[ ! -f ".env" ]]; then
            ...
            # Use Python for the substitution — sed's s|...|...|  delimiter
            # breaks silently when the secret value contains | or & (& is
            # "matched string" in sed replacement text; | is the delimiter
            # itself). Python re.sub has no such ambiguity.
            python3 - "${from_dir}/secrets.env" ".env" <<'PYEOF'
import sys, re
secrets_file, env_file = sys.argv[1], sys.argv[2]
with open(secrets_file) as f:
    pairs = {line.split('=',1)[0]: line.rstrip()
...
PYEOF
        ...
        log "[6/8] Skipping secrets (--skip-secrets or no secrets.env in source)"
    fi
    ...
    # 8. Bring app + worker + ai_worker back up so Alembic runs on entrypoint.
    log ""
    log "[8/8] Bringing app + worker + ai_worker back up — Alembic will run iris-ng's additive migrations on entrypoint..."
    docker compose -f "$IRIS_NG_COMPOSE" up -d app worker ai_worker

    # Wait for app to be healthy, then post-Alembic schema check.
    ...
    log "  iris-ng features (AI artifacts, working timeline, MISP sync, task links, team building,"
    log "  time tracking, evidence inventory, AI job queue, etc.): empty until used."
    log ""
