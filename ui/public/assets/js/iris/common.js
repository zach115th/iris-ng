...
function api_error(propagate_api_error, jqXHR, url) {
    if (propagate_api_error) {
        if(jqXHR.responseJSON && jqXHR.status == 400) {
            propagate_form_api_errors(jqXHR.responseJSON.data);
        } else {
...
function get_request_data_api(uri, data, propagate_api_error, beforeSend_fn) {
    return $.ajax({
        ...
        error: function(jqXHR) {
            api_error(propagate_api_error, jqXHR, this.url);
        }
    });
}
...
function exportInnerPng() {
    /* html2canvas is fetched ON FIRST CLICK — this is its only call site
       (the quick-sidebar "Export page as PNG" link) and the library is
       ~190K, so eager-loading it in every layout head bought nothing.
       Same-origin dynamic script, so the 'self' CSP covers it. */
    if (typeof html2canvas === 'function') { return _exportInnerPngRun(); }
    var s = document.createElement('script');
    s.src = '/static/assets/js/plugin/html2canvas/html2canvas.min.js';
    s.onload = _exportInnerPngRun;
    s.onerror = function () {
        if (typeof notify_error === 'function') {
            notify_error('Failed to load the PNG export library.');
        }
    };
    document.head.appendChild(s);
}

function _exportInnerPngRun() {
...
function get_showdown_convert() {
    return {
        makeHtml: function(text) {
            let sanitized = text.replace(/<img[^>]*>/gi, function(match) {
            ...
            let doc = parser.parseFromString('<div id="iris-md-root">' + html + '</div>', 'text/html');
            ...
            return doc.getElementById('iris-md-root').innerHTML;
        }
    };
}
...
/* One CSV cell, escaped for the product's own CSV importers.
 *
 * Shared by the IOC and asset exports so the two cannot drift apart on
 * quoting. Newlines are flattened deliberately: every one of those importers
 * runs splitlines() BEFORE handing the lines to csv.DictReader, and while the
 * reader does reassemble a quoted multi-line field, it joins the pieces with
 * NO separator — "line one\nline two" is read back as "line oneline two".
 * Flattening here keeps the text readable on a round trip instead of silently
 * welding words together. */
function iris_csv_cell(v) {
...
$('.toggle-sidebar').on('click', function() {
    if ($('.wrapper').hasClass('sidebar_minimize')) {
        ...
        get_request_api('/user/mini-sidebar/set/false')
            .then((data) => {
                // IRIS API envelope is {status, message, data} — checking
                // `data.success` here was the upstream bug that fired a red
                // "Mini sidebar disabled" error toast on every successful
                // toggle (the field doesn't exist, so the check was always
                // truthy). Compare against `status` instead.
                if (data.status !== 'success') {
                    notify_api_request_error(data);
                }
            });
...
$(document).ready(function(){
    ...
    // parse it, finds no csrf_token and rejects the request.
    // cid goes through the 5th argument -- putting it in the URI produced
    // '/context/set?cid=10?cid=9', because post_request_api appends its own.
    post_request_api('/context/set', JSON.stringify(data_sent), undefined, undefined, data_sent.ctx)
    .done((data) => {
            if (api_request_failed(data)) {
                return true;
            }
