...
/***    Sectors (iris-ng: catalog behind the sector pickers + metrics)    ***/

function iris_sector_form() {
    var form = $('form#form_new_sector').serializeObject();
    // Checkbox sits outside the WTForms fields; serializeObject drops
    // unchecked boxes entirely, so send the boolean explicitly.
    form['enabled'] = $('#sector_enabled').is(':checked');
    return form;
}

function add_sector() {
    url = '/manage/sectors/add/modal' + case_param();
    $('#modal_add_type_content').load(url, function (response, status, xhr) {
        if (status !== "success") {
             ajax_notify_error(xhr, url);
             return false;
        ...
        $('#submit_new_sector').on("click", function () {
            post_request_api('/manage/sectors/add', JSON.stringify(iris_sector_form()), true)
            .done((data) => {
                if(notify_auto_api(data)) {
...
$('#sector_table').dataTable({
    "ajax": {
      "url": `/manage/sectors/list${case_param()}`,
      "contentType": "application/json",
      "type": "GET",
      "data": function ( d ) {
        if (d.status == 'success') {
    ...
    "columns": [
        {
            "data": "name",
            "render": function ( data, type, row ) {
                return '<a href="#" onclick="sector_detail(\'' + row['id'] + '\');">' + sanitizeHTML(data) +'</a>';
            }
        },
        {
            "data": "slug",
        ...
        },
        {
            ...
            "render": function ( data, type, row ) {
                if (type === 'display') {
                    return data ? 'Yes' : '<span class="text-warning">No — hidden from pickers</span>';
                }
                return data;
            }
        }
...
function refresh_sector_table() {
  $('#sector_table').DataTable().ajax.reload();
  notify_success("Refreshed");
}

function sector_detail(sector_id) {
    url = '/manage/sectors/update/' + sector_id + '/modal' + case_param();
    $('#modal_add_type_content').load(url, function (response, status, xhr) {
        if (status !== "success") {
             ajax_notify_error(xhr, url);
             return false;
        ...
        $('#submit_new_sector').on("click", function () {
            post_request_api(`/manage/sectors/update/${sector_id}`, JSON.stringify(iris_sector_form()), true)
            .done((data) => {
                if(notify_auto_api(data)) {
                    refresh_sector_table();
                    $('#modal_add_type').modal('hide');
...
function delete_sector(id) {
    swal({
      title: "Are you sure?",
      text: "Deleting also drops metrics recognition for this sector's namespace unless another row or the built-in DHS/threatmatch forms cover it. Disable instead to retire it from the pickers only.",
      icon: "warning",
      buttons: true,
      dangerMode: true,
      confirmButtonColor: '#3085d6',
    ...
    .then((willDelete) => {
        if (willDelete) {
            post_request_api('/manage/sectors/delete/' + id)
            .done((data) => {
                if(notify_auto_api(data)) {
                    refresh_sector_table();
                    $('#modal_add_type').modal('hide');
