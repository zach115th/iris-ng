...
var _tm_sort_dir = localStorage.getItem('iris-tm-sort') || 'asc';

/* edit_in_event_desc() and add_event() moved to the shared
 * event_modal.js (loaded by this page BEFORE this file) so the Assets and
 * IOC pages can open the same add-event modal in place. */
...
function toggle_sort_direction() {
    _tm_sort_dir = _tm_sort_dir === 'asc' ? 'desc' : 'asc';
    localStorage.setItem('iris-tm-sort', _tm_sort_dir);
    apply_filtering();
}
...
function build_timeline(data) {
    ...
    var is_i = false;
    current_timeline = data.data.tim.slice().sort(function(a, b) {
        var da = new Date(a.event_date), db = new Date(b.event_date);
        return _tm_sort_dir === 'desc' ? db - da : da - db;
    });
    data.data.tim = current_timeline;
    let tmb = [];
    ...
    // Inject sort pill as a separate sibling next to the first date pill
    if (data.data.tim.length > 0) {
        var $firstBadge = $('#timeline_list .time-badge').first();
        if ($firstBadge.length) {
            var sortLabel = _tm_sort_dir === 'asc' ? 'Oldest first ▲' : 'Newest first ▼';
            var sortTitle = _tm_sort_dir === 'asc' ? 'Click to show newest first' : 'Click to show oldest first';
            $firstBadge.after(
                '<button id="iris-tm-sort-btn" class="btn btn-xs" ' +
                'style="margin-left:6px;font-size:10px;padding:1px 8px;background:rgba(139,92,246,0.15);border:1px solid rgba(139,92,246,0.35);color:#d4c4ff;vertical-align:middle;" ' +
                'onclick="toggle_sort_direction();" title="' + sortTitle + '">' + sortLabel + '</button>'
            );
        }
    }
...
$('input[name="csv_upload_dest"]').on('change', function () {
    const isWorking = $(this).val() === 'working';
    $('#csv_upload_date_filter').toggle(isWorking);
});

function upload_csv_events() {
    const modal_dlg = '#modal_upload_csv_events';
    const file_input = '#input_upload_csv_events';
    const dest = $('input[name="csv_upload_dest"]:checked').val();

    const file = $(file_input).get(0).files[0];
    if (!file) {
        swal('No file', 'Please choose a CSV file before uploading.', 'warning');
        return false;
    }

    if (dest === 'working') {
        // Route to the working-timeline import endpoint via FormData (multipart).
        const cid = get_caseid();
        const formData = new FormData();
        formData.append('file', file);
        formData.append('csrf_token', $('#csrf_token').val());
        const begin = $('#csv_upload_begin').val();
        const end = $('#csv_upload_end').val();
        if (begin) formData.append('begin_date', begin);
        if (end) formData.append('end_date', end);

        fetch(`/api/v2/cases/${cid}/working-timeline/import/master-csv`, {
            method: 'POST',
        ...
        .then(data => {
            if (data.imported !== undefined) {
                let msg = `${data.imported} event(s) staged in the working timeline.`;
                if (data.skipped_out_of_range) {
                    msg += ` ${data.skipped_out_of_range} skipped (outside date filter).`;
                }
                $(modal_dlg).modal('hide');
                irisWorkingTimeline.refresh();
                swal('Staged for review', msg, 'success');
            } else {
                const errMsg = (data.detail || data.message || JSON.stringify(data));
                swal('Import failed', errMsg, 'error');
            }
        })
        .catch(err => swal('Import failed', String(err), 'error'));

        return false;
    }

    // Default: master timeline (existing behaviour).
