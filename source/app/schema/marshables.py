...
class CaseAssetsSchema(ma.SQLAlchemyAutoSchema):
    ...
    evidence_links: List[int] = fields.List(fields.Integer, required=False)
...
class ServerSettingsSchema(ma.SQLAlchemyAutoSchema):
    ...
    # WRITE-ONLY: load_only keeps them out of every dump (settings GET, the
    # app.config['SERVER_SETTINGS'] cache), and the paired *_password_set
    # booleans tell the UI whether a secret is stored without revealing it.
    # The update route treats empty/masked submissions as "keep stored value".
    mail_ingest_enabled: Optional[bool] = fields.Boolean(required=False, allow_none=True)
    ...
    mail_imap_ssl: Optional[bool] = fields.Boolean(required=False, allow_none=True)
    mail_imap_username: Optional[str] = fields.String(required=False, allow_none=True)
    ...
    mail_smtp_username: Optional[str] = fields.String(required=False, allow_none=True)
    mail_smtp_password: Optional[str] = fields.String(required=False, allow_none=True, load_only=True)
    mail_smtp_from_addr: Optional[str] = fields.String(required=False, allow_none=True)
    mail_ai_triage_enabled: Optional[bool] = fields.Boolean(required=False, allow_none=True)
    email_notifications_enabled: Optional[bool] = fields.Boolean(required=False, allow_none=True)
    # Phase 5: org-wide notification defaults, {event_type: {in_app, email}}.
    notification_defaults: Optional[dict] = fields.Dict(required=False, allow_none=True)
    mail_imap_password_set = fields.Method("get_imap_password_set", dump_only=True)
    mail_smtp_password_set = fields.Method("get_smtp_password_set", dump_only=True)

    def get_imap_password_set(self, obj) -> bool:
        return bool(getattr(obj, 'mail_imap_password', None))

    def get_smtp_password_set(self, obj) -> bool:
        return bool(getattr(obj, 'mail_smtp_password', None))

    # iris-next: evidence drive retention policy (months). None = no policy.
    retention_months: Optional[int] = fields.Integer(required=False, allow_none=True)

    # iris-next: capacity planning settings. None = use defaults (window=3, target=2 months).
    capacity_planning_window_months: Optional[int] = fields.Integer(required=False, allow_none=True)
    capacity_planning_target_months: Optional[int] = fields.Integer(required=False, allow_none=True)
...
class SectorCatalogSchema(ma.SQLAlchemyAutoSchema):
    """iris-ng: schema for the admin-editable sector catalog.

    `slug` is the stable key customers reference (Client.dhs_sectors stores
    bare slugs comma-separated), so it is shape-validated to stay safe in
    that storage. `tag` must be a full MISP machine-tag —
    namespace:predicate="value" — because the picker emits it verbatim and
    the metrics recognition derives prefixes from it.
    """
    slug: str = auto_field('slug', required=True, allow_none=False,
                           validate=Length(min=2))
    name: str = auto_field('name', required=True, allow_none=False,
                           validate=Length(min=2))
    tag: str = auto_field('tag', required=True, allow_none=False,
                          validate=Length(min=5))
    enabled: bool = auto_field('enabled', required=False)
    ...
    @post_load
    def verify_sector(self, data, **kwargs):
        if not re.fullmatch(r'[a-z0-9][a-z0-9._-]*', data.slug or ''):
            raise marshmallow.exceptions.ValidationError(
                "Slug must be lowercase letters/digits/dots/dashes/underscores"
                " (it is stored comma-separated on customers)",
                field_name="slug")
        if not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._-]*:[^=\s"]+="[^"]+"',
                            data.tag or ''):
            raise marshmallow.exceptions.ValidationError(
                'Tag must be a MISP machine-tag: namespace:predicate="value"',
                field_name="tag")
        rival = SectorCatalog.query.filter(
            func.lower(SectorCatalog.slug) == func.lower(data.slug),
            SectorCatalog.id != data.id
        ).first()
        if rival:
            raise marshmallow.exceptions.ValidationError(
                "Sector slug already exists", field_name="slug")
        return data
...
_MAIL_RULE_CONDITION_FIELDS = ('subject', 'from', 'to', 'body')
...
def _validate_mail_rule_conditions(value):
    ...
    for i, cond in enumerate(value):
        if not isinstance(cond, dict):
            raise ValidationError(f'condition #{i + 1} must be an object')
        field = cond.get('field')
        regex = cond.get('regex')
        if field not in _MAIL_RULE_CONDITION_FIELDS:
...
class MailRuleSchema(ma.SQLAlchemyAutoSchema):
    """iris-ng v2 (Phase 1): mail-rule CRUD. FKs included as plain ids
    (customer_id/severity_id/classification_id resolve at runtime per the
    lookup-id project rule); rule_uuid and timestamps are server-owned."""
    name = fields.String(required=True, validate=validate.Length(min=1, max=256))
    action = fields.String(required=False,
                           validate=validate.OneOf(['create_alert', 'ignore']))
    conditions = fields.Raw(required=False, allow_none=True,
                            validate=_validate_mail_rule_conditions)
    priority = fields.Integer(required=False, allow_none=False)
    ...
    class Meta:
        ...
        include_fk = True
        unknown = EXCLUDE
        dump_only = ('id', 'rule_uuid', 'created_at', 'updated_at', 'created_by')


class MailIngestLogSchema(ma.SQLAlchemyAutoSchema):
    """Dump-only audit rows for the Recent Ingest Activity panel."""
    rule_name = fields.Function(lambda obj: obj.rule.name if obj.rule else None)
...
def _validate_clustering_conditions(value):
    """iris-ng v2 (Phase 2): match_conditions is a condition tree
    (business/condition_eval.py grammar). Validated at save time so a
    structurally broken rule cannot reach ingest (which additionally fails
    closed at evaluation time)."""
    from app.business.condition_eval import validate_tree
    problems = validate_tree(value)
    if problems:
        raise ValidationError(problems)


def _validate_correlation_keys(value):
    if value is None:
        return
    if not isinstance(value, list):
        raise ValidationError('correlation_keys must be a list of dotted paths')
    for i, key in enumerate(value):
        if not isinstance(key, str) or not key.strip():
            raise ValidationError(f'correlation key #{i + 1} must be a non-empty string')


class AlertClusteringRuleSchema(ma.SQLAlchemyAutoSchema):
    """iris-ng v2 (Phase 2): clustering-rule CRUD. match_conditions and
    correlation_keys are validated at save time; window_minutes must be
    positive (mirrors the CHECK so a bad value is a 400, not a 500)."""
    name = fields.String(required=True, validate=validate.Length(min=1, max=256))
    match_conditions = fields.Raw(required=False, allow_none=True,
                                  validate=_validate_clustering_conditions)
    correlation_keys = fields.Raw(required=False, allow_none=True,
                                  validate=_validate_correlation_keys)
    window_minutes = fields.Integer(required=False,
                                    validate=validate.Range(min=1, max=525600))
    priority = fields.Integer(required=False, allow_none=False)

    class Meta:
        model = AlertClusteringRule
        load_instance = True
        include_fk = True
        unknown = EXCLUDE
        dump_only = ('id', 'rule_uuid', 'created_at', 'updated_at', 'created_by')
...
class InvestigationFlowSchema(ma.SQLAlchemyAutoSchema):
    """iris-ng v2 (Phase 3): flow CRUD (scalar fields). Steps are NOT loaded
    here — the endpoint merges them by id (replace-on-PUT would cascade-drop
    analyst step states; merge preserves them, per the model docstring)."""
    name = fields.String(required=True, validate=validate.Length(min=1, max=256))
    target = fields.String(required=False,
                           validate=validate.OneOf(['alert', 'cluster', 'both']))
    match_conditions = fields.Raw(required=False, allow_none=True,
                                  validate=_validate_clustering_conditions)
    priority = fields.Integer(required=False, allow_none=False)
    ...
    class Meta:
        ...
        include_fk = True
        unknown = EXCLUDE
        dump_only = ('id', 'flow_uuid', 'created_at', 'updated_at', 'created_by')
