...
from app.iris_engine.secret_key import resolve_server_secrets
...
def run_post_init(development=False):
    ...
    if os.getenv("IRIS_WORKER") is None:
        ...
        with app.app_context():
            ...
            # Create base server settings if they don't exist; if the row
            # already exists, the function still runs to backfill any
            # newly-added empty columns (e.g. ai_backend_* on existing
            # installs upgrading past migration c3f1b87a902d).
            log.info("Ensuring base server settings exist + backfilling new fields")
            ...
            log.info("Backfilling IOC type taxonomy from resources/misp.attribute_types.json")
            backfill_ioc_type_taxonomy()
            ...
            if not prevent_objects:
                ...
                # iris-ng: sector catalog rides the same objects-repush flag,
                # so an admin who deleted a seed row can make it stick by
                # setting prevent_post_objects_repush, like classifications.
                log.info("Seeding sector catalog")
                create_safe_sectors()
            ...
            # Runs unconditionally, including when prevent_post_mod_repush is
            # set: it only refreshes modules that are already registered and
            # never re-adds one the admin deleted. A module whose code grew a
            # new configuration parameter would otherwise keep the schema it
            # was first registered with, forever.
            log.info("Reconciling module configurations")
            reconcile_module_configurations()
            ...
            # Replace the shipped placeholder SECRET_KEY with a generated,
            # database-persisted one. Must run here: after db.create_all() and
            # the alembic upgrade (so runtime_secret exists) and before the app
            # serves a request (so no cookie is signed with the public value).
            log.info("Resolving server secrets")
            resolve_server_secrets(app, db, log=log)
...
def create_safe_events_cats():
    ...
    iris-next: extends the upstream IRIS list (which is MITRE ATT&CK
    Enterprise tactics + 3 IRIS-specific entries) with the seven Unified
    Kill Chain (UKC v1.3, Paul Pols) phases that ATT&CK doesn't cover.
    All entries are added via create_safe so re-running this on existing
    deployments only inserts the missing rows — existing event_category
    foreign keys stay intact.

    The dropdown order matches insertion order (no explicit `order_by`
    in datamgmt.case.case_events_db.get_events_categories), so the UKC
    additions appear at the end. Reorder requires renumbering primary
    keys; not worth the migration risk for a UI nicety.
    """
    # IRIS-specific (not in any kill-chain model — represent process state)
    ...
    # MITRE ATT&CK Enterprise tactics (12 of 14 — Reconnaissance and
    # Resource Development are added below as part of UKC since UKC
    # also includes them as phases #1 and #2).
    ...
    # Unified Kill Chain v1.3 phases (Paul Pols, GPL v2 — see
    # docs/21-unified-kill-chain.md). Phases that share a name with an
    # ATT&CK tactic above (Persistence, Defense Evasion, C2, Discovery,
    # Privilege Escalation, Execution, Credential Access, Lateral
    # Movement, Collection, Exfiltration, Impact) are reused — IRIS
    # does not need duplicate categories. The seven below are UKC-only:
    create_safe(db.session, EventCategory, name="Reconnaissance")          # UKC #1 / ATT&CK TA0043
    create_safe(db.session, EventCategory, name="Resource Development")    # UKC #2 / ATT&CK TA0042
    create_safe(db.session, EventCategory, name="Delivery")                # UKC #3 — no ATT&CK equivalent (subset of ATT&CK Initial Access)
    create_safe(db.session, EventCategory, name="Social Engineering")      # UKC #4 — no ATT&CK equivalent (split from Exploitation)
    create_safe(db.session, EventCategory, name="Exploitation")            # UKC #5 — distinct from ATT&CK "Initial Access" (which bundles social eng + exploit)
    create_safe(db.session, EventCategory, name="Pivoting")                # UKC #9 — UKC's signature contribution (choke-point activity)
    create_safe(db.session, EventCategory, name="Objectives")              # UKC #18 — socio-technical strategic goal, above ATT&CK's scope


# iris-ng: seed list for the sector catalog — the 16 DHS CIIP sectors plus
# threatmatch Education, exactly the options the pickers hardcoded before the
# catalog existed. Slug-keyed UPSERT (never create_safe: name/tag are mutable
# non-key fields and admins may edit them — an upsert must not resurrect an
# edit, so only MISSING slugs are inserted and existing rows are untouched).
_SECTOR_SEED = [
    ('chemical', 'Chemical', 'dhs-ciip-sectors:DHS-critical-sectors="chemical"'),
    ('commercial-facilities', 'Commercial Facilities', 'dhs-ciip-sectors:DHS-critical-sectors="commercial-facilities"'),
    ('communications', 'Communications', 'dhs-ciip-sectors:DHS-critical-sectors="communications"'),
    ('critical-manufacturing', 'Critical Manufacturing', 'dhs-ciip-sectors:DHS-critical-sectors="critical-manufacturing"'),
    ('dams', 'Dams', 'dhs-ciip-sectors:DHS-critical-sectors="dams"'),
    ('dib', 'Defense Industrial Base', 'dhs-ciip-sectors:DHS-critical-sectors="dib"'),
    ('emergency-services', 'Emergency Services', 'dhs-ciip-sectors:DHS-critical-sectors="emergency-services"'),
    ('energy', 'Energy', 'dhs-ciip-sectors:DHS-critical-sectors="energy"'),
    ('financial-services', 'Financial Services', 'dhs-ciip-sectors:DHS-critical-sectors="financial-services"'),
    ('food-agriculture', 'Food & Agriculture', 'dhs-ciip-sectors:DHS-critical-sectors="food-agriculture"'),
    ('government-facilities', 'Government Facilities', 'dhs-ciip-sectors:DHS-critical-sectors="government-facilities"'),
    ('healthcare-public', 'Healthcare & Public Health', 'dhs-ciip-sectors:DHS-critical-sectors="healthcare-public"'),
    ('it', 'Information Technology', 'dhs-ciip-sectors:DHS-critical-sectors="it"'),
    ('nuclear', 'Nuclear', 'dhs-ciip-sectors:DHS-critical-sectors="nuclear"'),
    ('transport', 'Transportation Systems', 'dhs-ciip-sectors:DHS-critical-sectors="transport"'),
    ('water', 'Water Systems', 'dhs-ciip-sectors:DHS-critical-sectors="water"'),
    ('threatmatch-education', 'Education', 'threatmatch:sector="Education"'),
]


def create_safe_sectors():
    """Insert seed sectors whose slug is MISSING; never touch existing rows
    (admins own them — labels, tags and the enabled flag are all editable).
    A DELETED seed row is re-inserted at next boot unless the admin sets
    prevent_post_objects_repush (same contract as classifications); the
    edit-safe retirement is the enabled flag."""
    from app.models.models import SectorCatalog
    existing = {r[0] for r in db.session.query(SectorCatalog.slug).all()}
    added = 0
    for slug, name, tag in _SECTOR_SEED:
        if slug in existing:
            continue
        db.session.add(SectorCatalog(slug=slug, name=name, tag=tag, enabled=True))
        added += 1
    if added:
        db.session.commit()
...
# iris-ng: curated cyber-security / DFIR skill catalog seeded on boot. Grouped
# by category. (slug, name, category, description). Slug is the stable key —
# renaming a name/description in this list updates the existing row; it does NOT
# create a duplicate (see create_safe_skills). Admins can add more via the DB;
# the larger team-building / skill-based-assignment plan builds on this catalog.
_BASE_SKILLS = [
    # Digital Forensics
    ("disk-forensics", "Disk Forensics", "Digital Forensics", "Filesystem, MFT, and artifact analysis of disk images"),
    ("memory-forensics", "Memory Forensics", "Digital Forensics", "Volatile memory analysis (Volatility, Rekall)"),
    ("mobile-forensics", "Mobile Forensics", "Digital Forensics", "iOS / Android device acquisition and analysis"),
    ("network-forensics", "Network Forensics", "Digital Forensics", "PCAP, netflow, and protocol analysis"),
    ("cloud-forensics", "Cloud Forensics", "Digital Forensics", "AWS / Azure / GCP log and artifact analysis"),
    ("windows-forensics", "Windows Forensics", "Digital Forensics", "Registry, EVTX, prefetch, shimcache, and Windows artifacts"),
    ("linux-forensics", "Linux / Unix Forensics", "Digital Forensics", "Unix filesystem, logs, and persistence analysis"),
    ("macos-forensics", "macOS Forensics", "Digital Forensics", "macOS unified logs, FSEvents, and artifact analysis"),
    # Incident Response
    ("incident-response", "Incident Response", "Incident Response", "IR coordination, containment, and eradication"),
    ("threat-hunting", "Threat Hunting", "Incident Response", "Proactive hypothesis-driven hunting across telemetry"),
    ("log-analysis", "Log Analysis / SIEM", "Incident Response", "SIEM querying and large-scale log correlation"),
    ("edr-analysis", "EDR Analysis", "Incident Response", "Endpoint detection-and-response investigation and triage"),
    ("ransomware-response", "Ransomware Response", "Incident Response", "Ransomware containment, decryption, and recovery"),
    ("bec-investigation", "BEC / Email Investigation", "Incident Response", "Business email compromise and phishing investigation"),
    # Malware
    ("malware-static", "Static Malware Analysis", "Malware Analysis", "Disassembly and static binary analysis"),
    ("malware-dynamic", "Dynamic Malware Analysis", "Malware Analysis", "Sandbox / behavioral malware analysis"),
    ("reverse-engineering", "Reverse Engineering", "Malware Analysis", "Deep binary RE (IDA, Ghidra, debuggers)"),
    # Threat Intelligence
    ("cti", "Cyber Threat Intelligence", "Threat Intelligence", "CTI collection, analysis, and reporting"),
    ("misp", "MISP / Threat Sharing", "Threat Intelligence", "MISP administration and threat-data sharing"),
    ("attribution", "Threat Actor Attribution", "Threat Intelligence", "TTP clustering and actor attribution"),
    ("attack-mapping", "MITRE ATT&CK Mapping", "Threat Intelligence", "Mapping activity to ATT&CK techniques and tactics"),
    # Detection Engineering
    ("detection-engineering", "Detection Engineering", "Detection Engineering", "Building and tuning detections"),
    ("sigma-rules", "Sigma Rule Development", "Detection Engineering", "Authoring and testing Sigma detection rules"),
    ("yara-rules", "YARA Rule Development", "Detection Engineering", "Authoring YARA rules for malware/file matching"),
    # Offensive / Adversary Emulation
    ("penetration-testing", "Penetration Testing", "Offensive Security", "Network and application penetration testing"),
    ("red-teaming", "Red Teaming", "Offensive Security", "Adversary emulation and red-team operations"),
    ("purple-teaming", "Purple Teaming", "Offensive Security", "Collaborative detection validation exercises"),
    # Cloud / Platform
    ("cloud-security", "Cloud Security", "Cloud & Platform", "Cloud posture, IAM, and workload security"),
    ("container-security", "Container / Kubernetes Security", "Cloud & Platform", "Container and orchestration security"),
    ("ot-ics-security", "OT / ICS Security", "Cloud & Platform", "Operational-technology and ICS/SCADA security"),
    # Governance / Specialised
    ("legal-ediscovery", "Legal / eDiscovery", "Governance & Specialised", "Legal hold, eDiscovery, and chain of custody"),
    ("compliance", "Compliance / Regulatory", "Governance & Specialised", "Regulatory reporting (GDPR, HIPAA, PCI, etc.)"),
    ("crisis-comms", "Crisis Communications", "Governance & Specialised", "Stakeholder and executive incident communications"),
    ("scripting-automation", "Scripting / Automation", "Governance & Specialised", "Python / PowerShell tooling and automation"),
]


def create_safe_skills():
    """Seed the cyber-security skill catalog idempotently on `skill_slug`.

    Unlike create_safe (which matches on ALL kwargs and would re-insert when a
    name/description changes), this matches on the stable slug and UPDATES the
    other fields — so editing the catalog list above updates existing rows in
    place instead of creating duplicates. New slugs are inserted.
    """
    for slug, name, category, desc in _BASE_SKILLS:
        existing = Skill.query.filter(Skill.skill_slug == slug).first()
        if existing:
            existing.skill_name = name
            existing.skill_category = category
            existing.skill_description = desc
        else:
...
def create_safe_server_settings():
    # Seed AI defaults from env on first install. After the row exists, admins
    # edit values via /manage/settings — env vars are no longer authoritative
    # except as a one-time backfill when the columns are still empty (covers
    # existing installs upgrading past the c3f1b87a902d migration).
    env_ai_url = (os.environ.get('AI_BACKEND_URL') or '').strip()
...
def register_default_modules():
    modules = ['iris_vt_module', 'iris_misp_module', 'iris_check_module',
               ...
               'iris_misp_sync_module', 'iris_misp_cluster_module']
...
def reconcile_module_configurations():
    """Refresh every registered module's stored config schema from its code.

    IRIS caches a module's configuration schema in `iris_module.module_config`
    at registration time, and `register_module()` returns early with "Module
    already exists in Iris" on every later boot — so the stored schema is
    frozen at first registration. Adding a parameter to a module's
    `module_configuration` therefore has NO effect on an existing install: the
    new parameter never appears in the admin form, and any code doing
    `config['mandatory']` on a hand-added entry raises KeyError, which surfaces
    as a 500 on /manage/modules/list and an opaque DataTables Ajax error.

    This runs on every boot and reconciles the two:
      - parameters declared in code but missing from the DB are added with
        their default,
      - parameters no longer declared are dropped,
      - metadata (human name, description, type, mandatory, section, default)
        is refreshed from code,
      - the admin-set `value` is always preserved.

    Values are never logged — several are API keys.
    """
    for module in IrisModule.query.all():
        try:
            mod_inst, _ = instantiate_module_from_name(module.module_name)
            if not mod_inst:
                # Module registered in the DB but no longer importable. Leave
                # the row alone; deleting it would lose the admin's settings.
                continue
            declared = mod_inst.get_init_configuration()
        except Exception as exc:
            log.warning(f"Could not read configuration for {module.module_name}: {exc}")
            continue

        if not declared:
            continue

        # The row's own metadata (shown on /manage/modules) also freezes at
        # first registration — refresh it so an upgraded module displays its
        # real version instead of the one it registered with.
        try:
            for attr, val in (('module_version', mod_inst.get_module_version()),
                              ('interface_version', mod_inst.get_interface_version()),
                              ('module_description', mod_inst.get_module_description())):
                if val and getattr(module, attr, None) != str(val):
                    setattr(module, attr, str(val))
        except Exception as exc:
            log.warning(f"Could not refresh metadata for {module.module_name}: {exc}")

        stored = {
            p.get('param_name'): p
            for p in (module.module_config or [])
            if isinstance(p, dict) and p.get('param_name')
        }

        rebuilt = []
        added, dropped, retyped = [], [], []
        for entry in declared:
            ...
            if previous is not None and 'value' in previous:
                ...
                if previous.get('type') != param.get('type'):
                    retyped.append(name)
            else:
        ...
        declared_names = {p.get('param_name') for p in declared}
        dropped = [n for n in stored if n not in declared_names]

        if not (added or dropped or retyped) and len(rebuilt) == len(stored):
            continue

        module.module_config = rebuilt
        flag_modified(module, 'module_config')
        ...
        if retyped:
            detail.append(f"type changed (value kept) {retyped}")
        log.info(
            f"Reconciled configuration for {module.module_name}: "
            + "; ".join(detail or ["metadata refreshed"])
        )
    ...
    try:
        ...
        db.session.rollback()
        log.error(f"Could not persist module configuration reconciliation: {exc}")
