...
from app.iris_engine.reporter.docx_repair import repair_docx_for_word
...
class IrisMakeDocReport(IrisReportMaker):
    ...
    def generate_doc_report(self, doc_type):
        ...
        try:
            ...
            # docx_generator re-serialises the package through lxml, which emits
            # a single-quoted XML declaration that Word's OPC parser rejects, and
            # docxtpl can leave unbalanced TOC bookmarks. Both produce a valid zip
            # with well-formed XML that python-docx opens but Word refuses
            # ("Word experienced an error trying to open the file"). Normalise the
            # declaration and rebalance bookmarks before handing the file back.
            repair_docx_for_word(output_file_path)
