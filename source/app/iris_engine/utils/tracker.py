...
def track_activity(message, caseid=None, ctx_less=False, user_input=False, display_in_ui=True):
    ...
    # iris-ng v2: outside a request context (celery workers - e.g. the mail
    # poller) current_user resolves to None and the bare attribute access
    # raised AttributeError, killing the caller. getattr keeps request-context
    # behaviour identical and makes worker calls log as Anonymous - the ua row
    # itself is unaffected (user_id was already guarded above).
