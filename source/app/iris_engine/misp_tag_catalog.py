...
# Membership set built alongside the catalog so `has_tag()` is O(1). Reset
# whenever `_CATALOG` is rebuilt — see `reload_catalog()` below.
...
def _ensure_tag_set() -> set[str]:
    global _TAG_SET
    if _TAG_SET is not None:
        return _TAG_SET
    # Build the catalog OUTSIDE the lock — `_ensure_catalog()` acquires the
    # same lock and `threading.Lock` is not reentrant, so doing it inside
    # this `with _LOCK:` block deadlocks the worker on first call. Once we
    # have the catalog list, the set build itself is cheap and the inner
    # critical section just publishes the result.
    catalog = _ensure_catalog()
    with _LOCK:
        if _TAG_SET is None:
            _TAG_SET = {r["tag"] for r in catalog}
    return _TAG_SET
...
def reload_catalog() -> int:
    ...
    restarting the app.
    """
    global _CATALOG, _TAG_SET
    with _LOCK:
        _CATALOG = _build_catalog()
        _TAG_SET = {r["tag"] for r in _CATALOG}
        return len(_CATALOG)
...
def has_tag(tag: str) -> bool:
    """Return True iff `tag` is a machine-tag in the bundled catalog.

    Case-sensitive — MISP values like `Unauthorised-information-access`
    differ from lowercased forms. Use to validate a derived tag string
    (e.g. CERT-XLM auto-tagging from case classification names) before
    persisting it, so admin-added classifications outside the taxonomy
    don't generate fake tags.
    """
    if not tag:
        return False
    return tag in _ensure_tag_set()
