...
def _build_activity_payload(case_id: int) -> dict[str, Any]:
    """Cross-object recency signal for the "no activity in N hours" check.

    The synthesizer previously inferred inactivity from timeline event dates
    only — but a case can be actively worked via IOCs, assets, notes, tasks, or
    evidence without a new *timeline* event. This computes, per object type, the
    most recent "touch" timestamp from that type's best column, plus an overall
    last-activity time and the hours since (server-side, so the model doesn't do
    date math). Timeline activity uses `event_added` (when the event was logged)
    NOT `event_date` (the historical time the event describes) — a 2020 event
    logged today is recent *activity*.

    The `UserActivity` audit log is folded in as a backstop so any tracked case
    action (including ones without a per-object update column, e.g. IOC edits)
    counts. Returns naive-UTC ISO strings + hours, matching the app's UTC
    convention.
    """
    now = datetime.utcnow()

    # Per-object-type most-recent timestamp. Each uses coalesce(update, added)
    # so an object that was created-but-never-updated still counts.
    ...
    # IOCs have no update/added column on the model, so lean on the audit log
    # (every ioc create/update emits a track_activity row) for the IOC signal
    # and as an overall backstop for any other tracked action.
    ...
    # Overall = the most recent across every signal we have.
...
def _hashable_payload(payload: dict[str, Any]) -> dict[str, Any]:
    """Copy of the synthesis payload with wall-clock-derived activity fields
    ...
    if isinstance(activity, dict):
        a = {k: v for k, v in activity.items()
             if k not in ("now_utc", "hours_since_last_activity")}
...
def case_last_activity_at(case_id: int) -> datetime | None:
    ...
    Reuses the cross-object recency signal the synthesizer already computes, so
    staleness of a manual edit is measured against the same definition of
    "case activity" the summary itself uses. Cheap — six MAX() queries, no
    model call.
    """
    ...
    if not raw:
        return None
    # _build_activity_payload emits naive-UTC ISO strings with a 'Z' suffix.
...
def summary_edit_is_stale(artifact: CaseAiArtifact) -> bool:
    ...
    Signals "your correction may no longer reflect the case" without forcing
    a regeneration — the analyst decides. Never true for an unedited artifact.
    """
...
def save_summary_edit(case_id: int, content: str, user_id: int) -> CaseAiArtifact:
    ...
    The original model output stays in `content` so the edit is always
    reversible. Raises CaseSummaryError when there is no summary to edit —
    editing is a correction workflow, not a way to author one from scratch.
    """
...
def generate_case_summary(case_id: int, *, force: bool = False) -> CaseAiArtifact:
    ...
    synthesis_payload = {
        ...
        "counts": counts,
        # Cross-object recency so the synthesizer's "no activity in 48h" check
        # spans IOCs / assets / notes / timeline / tasks / evidence, not just
        # timeline events. Server-computed hours_since_last_activity means the
        # model doesn't infer recency from scattered timestamps.
        "activity": _build_activity_payload(case_id),
        "tasks": tasks,
    ...
    # Cache key must NOT include wall-clock-derived fields, or it changes on
    # every call and the synthesis cache never hits. `activity.now_utc` and the
    # `hours_*` deltas are recomputed each run from utcnow(); only the absolute
    # `at` timestamps change when real activity happens — and those alone are
    # enough to invalidate the cache when the case is touched. Strip the
    # volatile fields for hashing; the full payload (incl. hours) still goes to
    # the model so it doesn't have to do date math.
    synthesis_hash = _hash_inputs(
        synthesis_model, synthesis_prompt, _hashable_payload(synthesis_payload)
    )
