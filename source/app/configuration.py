...
class CeleryConfig:
    ...
    # iris-next: dedicated queue for async AI jobs (docs/19 §5b.3). The
    # iris-ai-worker container consumes ONLY this queue with --concurrency=1,
    # so AI calls (a) never compete with MISP-sync / module hooks on the
    # default queue, and (b) leave the worker one-at-a-time — the single
    # LM Studio GPU stream is the real bottleneck, so the queue bounds load.
    # Everything else stays on the default 'celery' queue unchanged.
    task_routes = {
        'app.iris_engine.ai.ai_jobs.run_ai_job': {'queue': 'ai_queue'},
    }
    # Enable RabbitMQ message priorities (1=high … 9=low) on the AI queue so
    # interactive features (chat) jump ahead of heavy ones (summary).
    #
    # IMPORTANT: scope x-max-priority to ai_queue ONLY. The global
    # `task_queue_max_priority` setting would apply x-max-priority:10 to EVERY
    # queue, including the long-lived default 'celery' queue. RabbitMQ queue
    # arguments are immutable, so redeclaring an already-existing 'celery' queue
    # (created before this setting) with a new arg raises
    #   406 PRECONDITION_FAILED - inequivalent arg 'x-max-priority' ...
    # which surfaces as a failure on the next celery dispatch (e.g. case import).
    # Declaring task_queues explicitly keeps the default queue untouched.
    from kombu import Queue
    task_queues = (
        Queue('celery'),  # default queue — NO x-max-priority (matches existing broker decl)
        Queue('ai_queue', queue_arguments={'x-max-priority': 10}),
    )
...
class Config:
    ...
    IRIS_VERSION = "IRIS-NG-v2.0.0" # DO NOT EDIT THIS LINE MANUALLY
    ...
    # OpenAI-compatible AI backend (used by Tier-1 AI features such as the
    # case-summary generator). Defaults to empty so features can detect
    # "AI not configured" and skip cleanly.
    AI_BACKEND_URL = os.environ.get('AI_BACKEND_URL', '')
    ...
    # Optional web analytics. Empty by default, so nothing is loaded and nothing
    # is contacted unless an operator opts in. Deliberately NOT tied to demo mode:
    # upstream hardcoded DFIR-IRIS's own Plausible instance behind the demo flag,
    # which meant enabling demo mode reported page views to a third party.
    #
    # The tag shape is Plausible/Umami compatible (script URL + data-domain).
    # ANALYTICS_ORIGIN is NOT read here -- nginx cannot see Flask config, so the
    # CSP allowlist entry is passed to the nginx container as its own env var.
    # Both come from the same .env key, so keep them pointing at the same host.
    ANALYTICS_SCRIPT_URL = config.load('IRIS', 'ANALYTICS_SCRIPT_URL', fallback='')
    ANALYTICS_SITE_ID = config.load('IRIS', 'ANALYTICS_SITE_ID', fallback='')

    # Cross-origin access, opt-in and empty by default.
    #
    # Upstream emitted `Access-Control-Allow-Origin: *` together with
    # `Access-Control-Allow-Credentials: true` on EVERY response. Browsers reject
    # that pair, so it never granted credentialed cross-origin reads -- but the
    # wildcard alone still let any site read unauthenticated responses and use a
    # visiting analyst's browser to confirm an internal-only host is reachable.
    #
    # Nothing in IRIS-NG needs it: the UI calls same-origin `/api/v2/...` paths,
    # and API clients, n8n and the LLM backends are all server-side HTTP, where
    # CORS does not apply. So the default emits no CORS headers at all.
    #
    # Set a comma-separated origin list to re-enable it for a genuine browser
    # client on another origin. The matching origin is echoed back verbatim --
    # never `*` -- which is what makes credentialed requests legal at all.
    CORS_ALLOWED_ORIGINS = [
        origin.strip()
        for origin in config.load('IRIS', 'CORS_ALLOWED_ORIGINS', fallback='').split(',')
        if origin.strip()
    ]
