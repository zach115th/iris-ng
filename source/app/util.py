...
def add_obj_history_entry(obj, action, commit=False, user=None):
    # iris-ng v2: `user` lets non-request contexts (the mail-rule poller in a
    # celery worker, where current_user resolves to None) attribute the entry
    # explicitly. Every pre-existing call site omits it and behaves exactly as
    # before. Pass a User ORM object.
    if user is None:
        user = current_user
