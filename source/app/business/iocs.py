...
def iocs_create(request_json, case_identifier):
    ...
    add_obj_history_entry(ioc, 'created', commit=True)
...
def iocs_update(ioc: Ioc, request_json: dict) -> (Ioc, str):
    ...
    try:
        ...
        # iris-next: record the edit in modification_history. The column exists on
        # Ioc and every other commentable object logs its lifecycle, but the IOC
        # update path never populated it — so there was no way to tell from the DB
        # whether an IOC had been edited, and the server-rendered History panel
        # used on timeline events could not be offered for IOCs.
        # Written before the commit so it lands in the same transaction.
        add_obj_history_entry(ioc_sc, 'ioc updated')
