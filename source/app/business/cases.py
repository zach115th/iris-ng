...
# These two namespaces are the FLOOR — recognition must survive an emptied
# catalog, and historical tags in these forms must always count. The sector
# CATALOG (Case Objects > Sectors) extends this set at runtime: any namespace
# an admin adds there is recognized without a code change.
...
def _sector_tag_prefixes():
    """Legacy floor + prefixes derived from every sector-catalog row
    (enabled or not — disabling retires a picker option, never recognition).
    Fail-soft: a catalog read error degrades to the legacy constants."""
...
def _case_has_sector_tag(tag_titles):
    """True if any tag is a recognized sector tag (catalog-driven)."""
    prefixes = _sector_tag_prefixes()
    return any(
        (t or '').startswith(prefixes) for t in tag_titles
    )
...
def _sector_slug_to_tag(slug):
    ...
    Catalog first (the authoritative mapping since the sector catalog
    shipped); the legacy expansion stays as the fallback so customers whose
    stored slug no longer has a catalog row keep resolving exactly as they
    did before the catalog existed."""
    slug = (slug or '').strip()
    try:
        from app.datamgmt.manage.manage_sectors_db import get_sector_tag_by_slug
        tag = get_sector_tag_by_slug(slug)
        if tag:
            return tag
    except Exception:
        pass
...
def cases_update(case_identifier, request_data):
    ...
    try:
        ...
        case_previous_owner_id = case_i.owner_id
        ...
        # Phase 5 notifications - post-commit, fail-soft by contract.
        # An ownership transfer notifies the NEW owner (case_assigned) and
        # suppresses the generic case_updated_own for the same save; any
        # other update notifies the current owner. notify() drops the actor,
        # so self-edits by the owner emit nothing.
        from app.business.notifications import notify
        if case.reviewer_id and case.reviewer_id != case_previous_reviewer_id:
            notify('case_review_requested', [case.reviewer_id],
                   f'You are asked to review case "{case.name}"',
                   object_type='case', object_id=case.case_id,
                   case_id=case.case_id, url=f'/case?cid={case.case_id}',
                   actor_id=current_user.id, keep_actor=True)
        if case.owner_id and case.owner_id != case_previous_owner_id:
            notify('case_assigned', [case.owner_id],
                   f'Case "{case.name}" assigned to you',
                   object_type='case', object_id=case.case_id,
                   case_id=case.case_id, url=f'/case?cid={case.case_id}',
                   actor_id=current_user.id)
        elif case.owner_id:
            notify('case_updated_own', [case.owner_id],
                   f'Case "{case.name}" was updated',
                   object_type='case', object_id=case.case_id,
                   case_id=case.case_id, url=f'/case?cid={case.case_id}',
                   actor_id=current_user.id)
...
def case_peek_payload(case_id):
    """The tiles + safe-rendered summary shown by the shared case-peek modal.

    One builder for every surface that peeks at a case (war-room Cases tab,
    IOC cross-case links, anything later) so the popup cannot drift between
    them. Callers own the ACL decision — this only reads.

    Returns None when the case does not exist.
    """
    from app.iris_engine.safe_markdown import render_markdown_safe
    from app.models.cases import Cases
    c = db.session.get(Cases, int(case_id))
    if c is None:
        return None
    return {
        'case_id': c.case_id, 'name': c.name, 'soc_id': c.soc_id,
        'state_name': c.state.state_name if c.state else None,
        'severity_name': c.severity.severity_name if c.severity else None,
        'client_name': c.client.name if c.client else None,
        'owner_name': c.owner.name if c.owner else None,
        'reviewer_name': c.reviewer.name if c.reviewer else None,
        'open_date': c.open_date, 'close_date': c.close_date,
        'tags': [t.tag_title for t in (c.tags or [])],
        'description_html': render_markdown_safe(c.description or ''),
    }
