...
def assets_create(case_identifier, request_json):
    ...
    if request_data.get('evidence_links') is not None:
        errors, _ = set_evidence_links(request_data.get('evidence_links'), asset.asset_id)
        if errors:
            raise BusinessProcessingError('Encountered errors while linking evidence. Asset has still been created.')
...
def get_assets_case(case_identifier):
    ...
    # iris-ng v2: linked evidence, same bulk-cache shape as ioc_links —
    # feeds the Assets tab's Evidence panel (additive payload field)
    from app.datamgmt.case.case_assets_db import get_assets_evidence_links
    cache_evidence_link = {}
    for ev in get_assets_evidence_links(case_identifier):
        cache_evidence_link.setdefault(ev.asset_id, []).append(ev._asdict())
    ...
    for asset in assets:
        ...
        asset['evidence_links'] = cache_evidence_link.get(asset['asset_id'])
...
def assets_update(asset: CaseAssets, request_json):
    ...
    if 'evidence_links' in request_data:
        errors, _ = set_evidence_links(request_data.get('evidence_links') or [], asset.asset_id)
        if errors:
            raise BusinessProcessingError('Encountered errors while linking evidence. Asset has still been updated.')
    ...
    if asset_schema:
        ...
        # iris-ng v2 (Phase 4): registry sync on every asset write path —
        # a rename/retype lands as a new registry identity, and a
        # compromise verdict can raise the registry status. Never raises.
        from app.datamgmt.manage.customer_assets_db import sync_case_asset
        sync_case_asset(asset_schema)
        return asset_schema
