...
def _render_md(text: str) -> str:
    """Render markdown to HTML with raw HTML escaped (tables still supported).

    Must stay sanitising: this output is injected into the DOM unescaped by the
    correlation drawer, so plain `mistletoe.markdown()` here is a stored-XSS
    sink -- note content is analyst-supplied. See iris_engine/safe_markdown.py.
    """
...
# longevity signal. A TLP:RED indicator is not more or less likely to be real,
# nor does it age differently - it is simply one you may not redistribute.
# (dashboard_metrics.py independently reached the same conclusion and excludes
# tlp:* as no-signal.)
#
# This used to add "and correlation filters to green/clear anyway, so red/amber
# never reach this code" - no longer true. Correlation display now covers every
# TLP, so red/amber indicators DO reach these functions and are scored normally.
# That is intended: the score describes the indicator, and the sharing decision
# is enforced separately on the outbound paths (_SHAREABLE_TLP_IDS).
# ---------------------------------------------------------------------------
...
#
# CHANGED: correlation DISPLAY no longer filters by TLP.
#
# The original restriction was green/clear only, on the reasoning that the
# Correlation tab is a shared dashboard. That over-corrected: every correlation
# query is already scoped to `_accessible_case_ids(user_id)`, so an analyst only
# ever sees indicators from cases they can already open — the TLP filter hid
# data from people entitled to it, and because the IOC modal defaults new IOCs
# to AMBER, it hid most of them by default.
#
# What TLP legitimately governs is REDISTRIBUTION, not internal visibility. So
# the restriction moved to the outbound paths, where it actually means
# something: the STIX exporter and the MISP cluster publisher both filter to
# _SHAREABLE_TLP_IDS explicitly. Those are the code paths that send indicators
# to a third party.
_SHAREABLE_TLP_IDS = (3, 4)  # green, clear — safe to publish externally

# Ordering by handling restriction, most restrictive first. Used to pick a
# single badge for a value that appears in several cases under different TLPs:
# the most restrictive wins, because that is the handling rule the analyst is
# bound by for that indicator overall.
...
_TLP_NAMES = {1: 'red', 2: 'amber', 3: 'green', 4: 'clear', 5: 'amber+strict'}

# Rank used for an IOC with no TLP set. NULL is possible: `ioc_tlp_id` has no
# column default, so any API client or n8n workflow that omits it writes NULL.
# Treated as most restrictive — an unlabelled indicator is not established as
# shareable, and it must never be silently published.
_TLP_RANK_UNSET = -1


def _most_restrictive_tlp(tlp_ids) -> tuple[Optional[int], str]:
    """Return ``(tlp_id, tlp_name)`` for the most restrictive TLP in the set.

    Returns ``(None, 'unset')`` when any appearance carries no TLP at all, so
    the UI shows the caveat rather than implying a level that was never chosen.
    """
    ids = [t for t in (tlp_ids or [])]
    if not ids or any(t is None for t in ids):
        return None, 'unset'
    worst = min(ids, key=lambda t: _TLP_RESTRICTIVENESS.get(t, _TLP_RANK_UNSET))
    return worst, _TLP_NAMES.get(worst, 'unset')


def is_shareable_tlp(tlp_id: Optional[int]) -> bool:
    """True when an indicator may leave the instance (STIX export, MISP push).

    NULL is NOT shareable — see `_TLP_RANK_UNSET`.
    """
    return tlp_id in _SHAREABLE_TLP_IDS
...
def _accessible_case_ids(user_id: int) -> list[int]:
    """Return the case IDs the user may see. An EMPTY list means the user
    can see NOTHING — never 'all'. The previous empty-to-None sentinel was a
    cross-tenant leak: ac_get_fast_user_cases_access returns [] for a user
    with zero case access (default deny_all), and None here meant every
    query ran UNSCOPED — a fresh zero-access account saw the whole org's
    correlations, IOC values and case names. Admins are unaffected: their
    effective-access rows are materialized, so their list is never empty.
    (Fixed in v2 Phase 6; the project conventions already forbade copying
    this sentinel.)"""
    return list(ac_get_fast_user_cases_access(user_id) or [])
...
def get_shared_ioc_pairs(
    ...
    case_ids (iris-ng v2 Phase 6, additive): restrict the computation to the
    given case set — used by war rooms to scope the shared-IOC view to a
    room's attached cases. Intersects with the viewer's ACL; behaviour is
    byte-identical to before when omitted.
    ...
    q = (
        db.session.query(
            ...
            # Every TLP this value carries across the cases it appears in. The
            # same indicator is routinely labelled differently per case, so the
            # badge shows the most restrictive of them (see _most_restrictive_tlp).
            # array_agg is deliberately NOT distinct-filtered on NULL: a NULL
            # appearance has to survive into the aggregate so it can force the
            # 'unset' caveat rather than being averaged away.
            func.array_agg(Ioc.ioc_tlp_id).label("tlp_ids"),
        )
    ...
    if case_ids is not None:
        q = q.filter(Ioc.case_id.in_(list(case_ids)))
    ...
    out = []
    for r in rows:
        tlp_id, tlp_name = _most_restrictive_tlp(r.tlp_ids)
        out.append({
            ...
            # Most restrictive TLP across appearances; `tlp_shareable` is what
            # the outbound paths key off, so the UI badge and the publish
            # decision can never disagree.
            "tlp_id": tlp_id,
            "tlp_name": tlp_name,
            "tlp_shareable": is_shareable_tlp(tlp_id),
        })
    return out
...
def get_ioc_cross_case_context(ioc_value: str, ioc_type_id: int, user_id: int) -> dict:
    ...
    for r in rows:
        ...
        appearances.append({
            ...
            # Per-case TLP: the same indicator is commonly labelled differently
            # in each case, and here there is one card per case, so each shows
            # its own rather than a rolled-up value.
            "tlp_id": r.ioc_tlp_id,
            "tlp_name": _TLP_NAMES.get(r.ioc_tlp_id, 'unset'),
            "tlp_shareable": is_shareable_tlp(r.ioc_tlp_id),
            "linked_notes": linked_notes,
        })

    overall_tlp_id, overall_tlp_name = _most_restrictive_tlp([r.ioc_tlp_id for r in rows])
    ...
    return {
        ...
        # Rolled up across every appearance, for a single header badge.
        "tlp_id": overall_tlp_id,
        "tlp_name": overall_tlp_name,
        "tlp_shareable": is_shareable_tlp(overall_tlp_id),
    }
...
def _extract_snippet(text: str, needle: str, max_chars: int = 300) -> str:
    ...
    The returned value is HTML, and the caller injects it into the DOM without
    escaping. It is only safe because `_render_md` sanitises -- do not swap that
    for a plain markdown renderer."""
