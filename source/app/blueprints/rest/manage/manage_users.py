...
from app.datamgmt.manage.manage_users_db import set_user_skills
...
from app.datamgmt.manage.manage_users_db import update_user_hourly_rate
...
@manage_users_rest_blueprint.route('/manage/users/<int:cur_id>/hourly-rate/update', methods=['POST'])
@ac_api_requires(Permissions.server_administrator)
def manage_user_hourly_rate_(cur_id):
    """iris-ng: set/clear a user's billing rate for case-cost estimation.
    Body: {"hourly_rate": <number>|null|""}. Empty/null clears the rate."""
    if not request.is_json:
        return response_error("Invalid request")
    ...
    raw = request.json.get('hourly_rate')
    if raw is None or (isinstance(raw, str) and raw.strip() == ''):
        ...
        try:
            ...
            return response_error("Hourly rate must be a number")
        if rate < 0:
            return response_error("Hourly rate cannot be negative")

    update_user_hourly_rate(user, rate)
    track_activity(f"hourly rate of user {cur_id} updated", ctx_less=True)

    return response_success("User hourly rate updated", data=get_user_details(cur_id))


@manage_users_rest_blueprint.route('/manage/users/<int:cur_id>/skills/update', methods=['POST'])
@ac_api_requires(Permissions.server_administrator)
def manage_user_skills_(cur_id):
    """iris-ng: replace a user's enabled cyber-security skill set.
    Body: {"skill_ids": [<int>, ...]}. An empty list clears all skills."""
    if not request.is_json:
        return response_error("Invalid request")
    ...
    if not isinstance(skill_ids, list):
        return response_error("Expected a list of skill IDs")

    set_user_skills(cur_id, skill_ids)
    track_activity(f"skills of user {cur_id} updated", ctx_less=True)

    return response_success("User skills updated", data=get_user_details(cur_id))
