...
from app.datamgmt.manage.manage_case_templates_db import case_template_to_dict
...
@manage_case_templates_rest_blueprint.route('/manage/case-templates/<int:cur_id>', methods=['GET'])
@ac_api_requires(Permissions.case_templates_read)
def get_case_template(cur_id):
    """Full editable JSON of one case template — additive iris-ng endpoint for
    the v3 editor page. Same builder as the legacy modal (case_template_to_dict),
    so the two surfaces cannot drift.

    Returns:
        Response: the template's editable fields plus its id
    """
    case_template = get_case_template_by_id(cur_id)
    if not case_template:
        return response_error(f"Invalid Case template ID {cur_id}")

    data = case_template_to_dict(case_template)
    data['id'] = case_template.id
