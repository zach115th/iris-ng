...
def alerts_add_route() -> Response:
    ...
    # iris-ng: the ingest body lives in business/alerts_ingest.py so the API
    # route and the mail-rule poller share ONE code path (and one post-create
    # pipeline). Request/response behaviour is unchanged: the business layer
    # raises, this wrapper emits the exact same response_error payloads the
    # inlined code produced (verified byte-for-byte by a canned-alert probe).
    try:
        new_alert = create_alert_from_payload(request.get_json(),
                                              user_id=current_user.id)

        # Return the newly created alert as JSON
        return response_success(data=AlertSchema().dump(new_alert))
...
def alerts_update_route(alert_id) -> Response:
    ...
    try:
        ...
        # Phase 5: capture the pre-update owner for the assignment notification.
        previous_owner_id = alert.alert_owner_id
        ...
        # Phase 5: assignment notification - post-commit, fail-soft by contract.
        if updated_alert.alert_owner_id and updated_alert.alert_owner_id != previous_owner_id:
            from app.business.notifications import notify
            notify('alert_assigned', [updated_alert.alert_owner_id],
                   f'Alert #{alert_id} "{updated_alert.alert_title}" assigned to you',
                   object_type='alert', object_id=alert_id,
                   url=f'/alerts?alert_ids={alert_id}',
...
def alerts_batch_update_route() -> Response:
    ...
    for alert_id in alert_ids:
        ...
        try:
            ...
            # Phase 5: assignment notification - post-commit, fail-soft.
            if alert.alert_owner_id and alert.alert_owner_id != previous_owner_id:
                from app.business.notifications import notify
                notify('alert_assigned', [alert.alert_owner_id],
                       f'Alert #{alert.alert_id} "{alert.alert_title}" assigned to you',
                       object_type='alert', object_id=alert.alert_id,
                       url=f'/alerts?alert_ids={alert.alert_id}',
                       actor_id=current_user.id)
...
def alerts_escalate_route(alert_id) -> Response:
    ...
    try:
        ...
        # Phase 5: escalation notification to the alert owner - fail-soft.
        from app.business.notifications import notify
        notify('alert_escalated', [alert.alert_owner_id],
               f'Alert #{alert.alert_id} "{alert.alert_title}" escalated to case #{case.case_id}',
               object_type='alert', object_id=alert.alert_id, case_id=case.case_id,
               url=f'/case?cid={case.case_id}',
               actor_id=current_user.id)
...
def case_comment_add(alert_id):
    ...
    try:
        ...
        # Phase 5: @login mentions notify - fail-soft, ACL-bounded to users
        # with access to the alert's client.
        from app.business.notifications import client_access_filter, notify_mentions, scan_mentions
        mentioned = client_access_filter(scan_mentions(comment.comment_text),
                                         alert.alert_customer_id)
        notify_mentions(comment.comment_text,
                        f'{current_user.name} mentioned you on alert #{alert.alert_id} "{alert.alert_title}"',
                        body=(comment.comment_text or '')[:280],
                        object_type='alert', object_id=alert.alert_id,
                        url=f'/alerts?alert_ids={alert.alert_id}',
                        actor_id=current_user.id,
                        allowed_user_ids=mentioned)
