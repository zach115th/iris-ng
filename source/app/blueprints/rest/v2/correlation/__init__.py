...
def _build_ioc_records(pairs_for_cluster: list[dict]) -> list[dict]:
    ...
    published. The pair list already comes from the correlation query, which
    restricts to TLP:GREEN / TLP:CLEAR, so no additional TLP filtering is needed
    here; if that ever changes this function must filter explicitly.
    """
...
def cluster_misp_push(cluster_id: str):
    ...
    ioc_records = _build_ioc_records(pairs_for_cluster)
    if not ioc_records:
        return response_api_error(
