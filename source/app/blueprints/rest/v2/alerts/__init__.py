...
from app.blueprints.rest.endpoints import response_api_success, response_api_error, response_api_not_found
...
from app.iris_engine.ai.case_template_suggester import CaseTemplateSuggesterError
from app.iris_engine.ai.case_template_suggester import suggest_case_template
...
@alerts_blueprint.post('/<int:alert_id>/ai/case-template-suggestion')
@ac_api_requires(Permissions.alerts_read)
def suggest_case_template_endpoint(alert_id: int) -> Response:
    """Suggest the best-fitting CaseTemplate for an alert about to be escalated.

    Stateless / not cached — the case doesn't exist yet at suggestion time
    (the analyst is still in the escalation modal). The modal fires this
    on open and either auto-selects the dropdown option (and renders a
    "Suggested by AI" chip) or shows a "no confident match" status.

    The body is empty — the alert_id in the URL is the only input. The
    orchestrator pulls title/description/source/severity/classification/
    tags + attached IOCs and assets directly from the Alert ORM.

    Returns: {
      suggestion: {
        template_id: int,           # validated against CaseTemplate catalog
        template_name: str,         # canonical display_name from DB
    ...
    except CaseTemplateSuggesterError as exc:
