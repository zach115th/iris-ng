...
from app.blueprints.rest.v2.cases.notifications import case_notifications_blueprint
...
@cases_blueprint.route('/<int:case_identifier>/peek', methods=['GET'])
@ac_api_requires()
def get_case_peek(case_identifier):
    """Tiles + safe-rendered summary for the shared case-peek modal.

    Read access to THAT case is required — a caller reaching this for a case
    it cannot see gets a denial, never a peek. The war-room Cases tab has its
    own room-scoped route; both build the body from the same function.
    """
    from app.models.authorization import CaseAccessLevel
    ...
    out = case_peek_payload(case_identifier)
    if out is None:
        return response_api_not_found()
    return response_api_success(out)


@cases_blueprint.route('/<int:case_identifier>/header', methods=['GET'])
@ac_api_requires()
def get_case_header(case_identifier):
    """Case-shell header payload (v3 in-page case header): identity, meta,
    chips, object counts, and people-on-case. Read access suffices — this
    only aggregates what the case pages already show."""
    from app.models.authorization import CaseAccessLevel
    if not ac_fast_check_current_user_has_case_access(
            case_identifier,
            [CaseAccessLevel.read_only, CaseAccessLevel.full_access]):
    ...
    if case is None:
        return response_api_not_found()
    from app.business.notifications import case_access_user_ids
    ...
    counts = {
        'assets': CaseAssets.query.filter_by(case_id=case_identifier).count(),
        'iocs': Ioc.query.filter_by(case_id=case_identifier).count(),
        'tasks': CaseTasks.query.filter_by(
            task_case_id=case_identifier).count(),
        'evidence': CaseReceivedFile.query.filter_by(
            case_id=case_identifier).count(),
    }
    uids = case_access_user_ids(case_identifier)
    people = []
    if uids:
        people = [u.name for u in User.query.filter(
            User.id.in_(uids), User.active.is_(True)).all()]
    return response_api_success({
        'case_id': case.case_id, 'name': case.name, 'soc_id': case.soc_id,
        ...
        'state_name': case.state.state_name if case.state else None,
        'severity_name': (case.severity.severity_name
                          if case.severity else None),
        'classification': (case.classification.name_expanded
                           if case.classification else None),
        'alerts_count': len(case.alerts or []),
        'tags': [t.tag_title for t in (case.tags or [])],
        'review_status': (case.review_status.status_name
                          if case.review_status else None),
        'reviewer_name': case.reviewer.name if case.reviewer else None,
        'counts': counts,
        'people': people[:12],
