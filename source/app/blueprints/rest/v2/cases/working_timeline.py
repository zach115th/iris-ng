...
Surface area:
    ...
    POST   /api/v2/cases/<cid>/working-timeline/import/eztools     (multipart)
    POST   /api/v2/cases/<cid>/working-timeline/import/master-csv  (multipart)
...
# --- IMPORT DATE-WINDOW FILTER -------------------------------------------

class _DateWindowError(Exception):
    """Raised when a begin/end date filter can't be parsed."""


def _parse_day_bound(value: str | None, *, end: bool) -> datetime | None:
    """Parse a `YYYY-MM-DD` date filter into an inclusive UTC datetime bound.

    Both ingest sources store event_date as a *naive* datetime representing
    UTC wall-clock (see `_iso_utc`), so the bounds are naive UTC too:
      * begin → that day at 00:00:00.000000 (inclusive)
      * end   → that day at 23:59:59.999999 (inclusive)

    Returns None when no value is supplied (filter inactive on that side).
    Accepts the HTML5 <input type="date"> value verbatim. We also tolerate a
    full ISO datetime string and just take its date component, so a future
    datetime-precision picker degrades gracefully to whole-day bounds.
    """
    if value is None:
        return None
    value = value.strip()
    if not value:
        return None
    # Take the date component only (date-only bounds per the agreed design).
    date_part = value.split('T')[0].split(' ')[0]
    try:
        d = datetime.strptime(date_part, '%Y-%m-%d').date()
    except ValueError:
        raise _DateWindowError(
...
def _read_date_window():
    """Pull optional begin/end date filters off the request (form or JSON).

    Returns ``(begin_dt, end_dt)`` — either may be None. Raises
    ``_DateWindowError`` on a malformed value or an inverted range.
    """
    begin_raw = request.form.get('begin_date')
    end_raw = request.form.get('end_date')
    if begin_raw is None and end_raw is None:
        body = request.get_json(silent=True) or {}
        begin_raw = body.get('begin_date')
        end_raw = body.get('end_date')

    begin_dt = _parse_day_bound(begin_raw, end=False)
    end_dt = _parse_day_bound(end_raw, end=True)
    if begin_dt and end_dt and begin_dt > end_dt:
        raise _DateWindowError(
            'Begin date is after end date — nothing would be imported.'
        )
    return begin_dt, end_dt


def _filter_rows_by_window(parsed, begin_dt, end_dt):
    """Drop parsed rows whose event_date falls outside [begin, end].

    Rows with no parseable timestamp (event_date is None) are KEPT regardless
    of the filter — a date filter must never silently swallow data we couldn't
    place on the timeline. (In practice both parsers already drop None-date
    rows, but we guard here so the contract holds for any future source.)

    Returns ``(kept_rows, dropped_count)``.
    """
    if not begin_dt and not end_dt:
        return parsed, 0
    kept = []
    dropped = 0
    for row in parsed:
        ts = row.get('event_date')
        if ts is None:
            kept.append(row)
            continue
        if begin_dt and ts < begin_dt:
            dropped += 1
            continue
...
def import_hayabusa(case_identifier):
    ...
    try:
        begin_dt, end_dt = _read_date_window()
...
@case_working_timeline_blueprint.post('/import/master-csv')
@ac_api_requires()
def import_master_csv(case_identifier):
    """Upload a master-timeline CSV export and stage it as pending working events.

    Accepts ``multipart/form-data`` with a ``file`` field, or a JSON body
    with a ``csv`` string field.  The CSV must use the canonical round-trip
    header produced by "Download as CSV":
        event_date,event_tz,event_title,event_category,event_content,
        event_raw,event_source,event_assets,event_iocs,event_tags

    Assets and IOCs from the CSV are preserved in ``event_raw`` for
    analyst reference but are not auto-linked — the analyst reviews each
    event in the working timeline and promotes the ones they want, at
    which point the normal promote flow handles asset / IOC resolution.
    """
    denied = _require_full_access(case_identifier)
    if denied is not None:
        return denied
    ...
    try:
        batch_id, parsed = parse_master_csv(csv_bytes, case_identifier)
    except MasterCsvParseError as exc:
        return response_api_error(str(exc))
    ...
    if skipped_out_of_range:
        window_note = f'; {skipped_out_of_range} skipped outside the date filter'
    track_activity(
        f'imported {inserted} master-timeline CSV event(s) into the working timeline '
        f'(batch {batch_id}){window_note}',
        caseid=case_identifier,
    )
...
def promote_working_event(case_identifier, working_id):
    ...
    promoted.event_is_flagged = True
    # iris-ng: stamp provenance into the event history so the master-timeline
    # history button can show this event was promoted (not manually added),
    # and from which ingest source.
    add_obj_history_entry(promoted, f'promoted from working timeline ({working.source})')
