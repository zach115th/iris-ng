...
from app.business.case_notifications import case_activity_log
...
def _denied_or_missing(case_identifier):
    """Access + existence gate, or None when the caller may proceed.

    Existence is checked SEPARATELY because the admin access check
    short-circuits True for ids that do not exist - so without this every
    route here answered 200 with an empty payload for any case id at all,
    while the sibling /header and /peek routes on the same resource 404.
    Existence is data; the two answers should not be indistinguishable.
    """
    if not ac_fast_check_current_user_has_case_access(case_identifier, _READ_LEVELS):
        return ac_api_return_access_denied(caseid=case_identifier)
    if get_case(case_identifier) is None:
...
def get_notifications(case_identifier):
    denied = _denied_or_missing(case_identifier)
...
@case_notifications_blueprint.get('/activity-log')
@ac_api_requires()
def get_case_activity_log(case_identifier):
    """The v3 "Live case log", optionally scoped to one view's category.

    `category` is validated against the known set rather than silently
    ignored: a typo that fell through to "everything" would look like the
    scoping had simply been forgotten.
    """
    denied = _denied_or_missing(case_identifier)
    if denied is not None:
        return denied
    ...
    try:
        return response_api_success(
            case_activity_log(case_identifier, category=category)
        )
