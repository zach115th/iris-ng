...
from app.datamgmt.case.case_iocs_db import get_iocs_misp_links
from app.datamgmt.case.case_iocs_db import get_cases_brief
...
@case_iocs_blueprint.get('/links')
@ac_api_requires()
def get_case_iocs_links(case_identifier):
    """Bulk note + asset links for every IOC in the case.

    A separate endpoint rather than extra fields on IocSchemaForAPIV2:
    that schema is the public v2 IOC shape and is dumped by several
    endpoints, so widening it changes what every API client receives.
    This mirrors the existing bulk /tasks/links route — one round trip
    instead of N per-IOC lookups.

    Returns {"<ioc_id>": {"note_links": [...], "asset_links": [...]}}.
    """
    ...
    ioc_ids = [i.ioc_id for i in Ioc.query.with_entities(Ioc.ioc_id)
               .filter(Ioc.case_id == case_identifier).all()]

    notes_by_ioc = get_iocs_note_links(ioc_ids)
    assets_by_ioc = get_iocs_asset_links(ioc_ids)
    misp_by_ioc = get_iocs_misp_links(ioc_ids)

    # Cross-case appearances, enriched for the Cases tab's cards. The ACL
    # lives in get_ioc_links_bulk (it resolves against the CURRENT user), so
    # only cases this viewer may see are described here; get_cases_brief is
    # then a plain read of exactly those ids.
    xcase_by_ioc = get_ioc_links_bulk(ioc_ids)
    brief = get_cases_brief({row['case_id']
                             for rows in xcase_by_ioc.values()
                             for row in rows})

    return response_api_success({
        str(i): {'note_links': notes_by_ioc.get(i, []),
                 'asset_links': assets_by_ioc.get(i, []),
                 # null (not {}) when never synced — "not published" and
                 # "published, details unknown" must stay distinguishable
                 'misp': misp_by_ioc.get(i),
                 'case_links': [brief.get(row['case_id'],
                                          {'case_id': row['case_id'],
                                           'name': row.get('case_name'),
                                           'client_name': row.get('client_name')})
                                for row in xcase_by_ioc.get(i, [])]}
        for i in ioc_ids
    })
