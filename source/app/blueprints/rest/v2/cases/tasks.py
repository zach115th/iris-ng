...
@case_tasks_blueprint.get('/links')
@ac_api_requires()
def list_all_case_task_links(case_identifier):
    """Return every task link in the case in canonical forward form.

    One round-trip lets a client build the full dependency hierarchy
    (the per-task ``/<id>/links`` endpoint would need N requests). Each
    edge is `from_task_id <link_type> to_task_id` as stored — the caller
    derives inverse views ('is blocked by', etc.) itself.

    Response shape:
        {
          "links": [
            {"from_task_id": 1, "to_task_id": 2, "link_type": "blocks"},
            ...
          ]
        }
    """
    ...
    rows = (
        CaseTaskLink.query.with_entities(
            CaseTaskLink.from_task_id,
            CaseTaskLink.to_task_id,
            CaseTaskLink.link_type,
        )
        .filter(CaseTaskLink.case_id == case_identifier)
        .all()
    )
    links = [
        {'from_task_id': r.from_task_id, 'to_task_id': r.to_task_id, 'link_type': r.link_type}
        for r in rows
    ]
    return response_api_success({'links': links})
