...
def download_case_activity(report_id, caseid):
    ...
    if report_id:
        ...
        if report:
            ...
            # Reports are generated fresh on every request and the URL is stable
            # (same report_id + cid). Without no-store the browser caches the
            # download and re-serves a stale copy on the next click.
