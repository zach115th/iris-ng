...
def case_comment_add(cur_id, caseid):
    try:
        ...
        notify_mentions(comment.comment_text,
                        f'{current_user.name} mentioned you on event "{event.event_title}" (case #{caseid})',
                        body=(comment.comment_text or '')[:280],
                        object_type='event', object_id=event.event_id, case_id=caseid,
                        url=f'/case/timeline?cid={caseid}',
...
def event_flag(cur_id, caseid):
    ...
    # iris-ng: record flag/unflag in the event history so reviewers can see who
    # cleared the "needs review" flag (and when). Unflagging is the analyst
    # marking the event as reviewed.
    add_obj_history_entry(event, 'flagged' if event.event_is_flagged else 'unflagged')
    db.session.commit()
...
def case_add_event(caseid):
    try:
        ...
        # iris-ng: distinguish manual creation from working-timeline promotion
        # so the event history button can show provenance.
        add_obj_history_entry(event, 'created (manual)')
    ...
    try:
        ...
        for row in csv_data:
            ...
            for asset_name in event_assets.split(";"):
                if asset_name == '':
                    continue
                # Accept both "name" and "name (Type)" — the latter is emitted
                # by the working-timeline parsers (Hayabusa / EZ Tools) and may
                # appear when a master-timeline CSV was built from promoted events.
                asset = get_asset_by_name(asset_name, caseid)
                if asset is None and asset_name.endswith(')') and ' (' in asset_name:
                    bare_name = asset_name[:asset_name.rfind(' (')]
                    asset = get_asset_by_name(bare_name, caseid)
                if asset:
