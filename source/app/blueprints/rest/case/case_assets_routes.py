...
def case_filter_assets(caseid):
    ...
    cache_evidence_link = {}
    for ev in get_assets_evidence_links(caseid):
    ...
    for a in assets:
        ...
        a['evidence_links'] = cache_evidence_link.get(a['asset_id'])
...
def case_comment_asset_add(cur_id, caseid):
    try:
        ...
        notify_mentions(comment.comment_text,
                        f'{current_user.name} mentioned you on asset "{asset.asset_name}" (case #{caseid})',
                        body=(comment.comment_text or '')[:280],
                        object_type='asset', object_id=asset.asset_id, case_id=caseid,
                        url=f'/case/assets?cid={caseid}&shared={asset.asset_id}',
