...
def case_comment_note_add(cur_id, caseid):
    try:
        ...
        notify_mentions(comment.comment_text,
                        f'{current_user.name} mentioned you on note "{note.note_title}" (case #{caseid})',
                        body=(comment.comment_text or '')[:280],
                        object_type='note', object_id=note.note_id, case_id=caseid,
                        url=f'/case/notes?cid={caseid}&shared={note.note_id}',
