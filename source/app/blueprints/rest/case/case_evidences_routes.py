...
from app.datamgmt.case.case_rfiles_db import get_evidence_asset_ids_bulk
...
def case_list_rfiles(caseid):
    ...
    # iris-next: physical_location lives on the linked EvidenceDrive now, not on
    # the evidence row (removed from the Register Evidence modal 2026-06-27). The
    # row's own physical_location column is deprecated and NULL for anything
    # registered since, so the list column renders blank even when the drive has
    # a location. Surface the drive's location; fall back to the row's own value
    # for legacy rows with no drive.
    drive_location_by_evidence_id = {
        evidence.id: evidence.drive.physical_location
        for evidence in crf
        if evidence.drive is not None and evidence.drive.physical_location
    }
    # iris-next: the dumped `assets` field carries EvidenceAssetLink ROW ids,
    # not asset ids — anything resolving assets from it looks up the wrong id
    # space. Additive `asset_links` carries the REAL asset ids (one bulk
    # query); `assets` is left as-is for existing consumers.
    asset_ids_by_evidence = get_evidence_asset_ids_bulk(
        [row.get('id') for row in evidences])

    for row in evidences:
        drive_location = drive_location_by_evidence_id.get(row.get('id'))
        if drive_location:
            row['physical_location'] = drive_location
        row['asset_links'] = asset_ids_by_evidence.get(row.get('id'), [])
...
def case_comment_evidence_add(cur_id, caseid):
    try:
        ...
        # Phase 5: @login mentions notify - fail-soft, ACL-bounded to case users.
        from app.business.notifications import case_access_user_ids, notify_mentions
        notify_mentions(comment.comment_text,
                        f'{current_user.name} mentioned you on evidence "{evidence.filename}" (case #{caseid})',
                        body=(comment.comment_text or '')[:280],
                        object_type='evidence', object_id=evidence.id, case_id=caseid,
                        url=f'/case/evidences?cid={caseid}',
                        actor_id=current_user.id,
                        allowed_user_ids=case_access_user_ids(caseid))
