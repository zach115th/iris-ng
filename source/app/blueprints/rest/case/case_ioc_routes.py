...
def case_list_ioc(caseid):
    ...
    # Fetch all cross-case links in a single self-join query instead of N individual lookups
    ioc_ids = [ioc.ioc_id for ioc in iocs]
    links_by_ioc = get_ioc_links_bulk(ioc_ids)

    # iris-ng v2: note provenance (ioc_note_link) in one bulk query —
    # additive note_links field, feeds the note pills on IOC surfaces.
    # Shared helper, so this and the v2 bulk links endpoint cannot drift.
    ...
    for ioc in iocs:
        out = ioc._asdict()
        out['link'] = links_by_ioc.get(ioc.ioc_id, [])
        out['note_links'] = notes_by_ioc.get(ioc.ioc_id, [])
...
def case_comment_ioc_add(cur_id, caseid):
    try:
        ...
        notify_mentions(comment.comment_text,
                        f'{current_user.name} mentioned you on IOC "{ioc.ioc_value}" (case #{ioc.case_id})',
                        body=(comment.comment_text or '')[:280],
                        object_type='ioc', object_id=ioc.ioc_id, case_id=ioc.case_id,
                        url=f'/case/ioc?cid={ioc.case_id}',
                        actor_id=current_user.id,
                        allowed_user_ids=case_access_user_ids(ioc.case_id))
