...
@profile_rest_blueprint.route('/user/hourly-rate/update', methods=['POST'])
@ac_api_requires()
def profile_set_hourly_rate():
    """iris-ng: self-service — the current user sets/clears their own billing
    rate. Body: {"hourly_rate": <number>|null|""}. Empty/null clears it."""
    ...
    track_activity("user {} updated its own hourly rate".format(user.user))

    return response_success(
        "Hourly rate updated",
        data={"user_hourly_rate": float(user.hourly_rate) if user.hourly_rate is not None else None}
    )


@profile_rest_blueprint.route('/user/skills/update', methods=['POST'])
@ac_api_requires()
def profile_set_skills():
    """iris-ng: self-service — the current user sets their own enabled skill
    set. Body: {"skill_ids": [<int>, ...]}. Empty list clears all."""
    if not request.is_json:
        return response_error("Invalid request")
    ...
    enabled = set_user_skills(current_user.id, skill_ids)
    track_activity("user {} updated its own skills".format(current_user.user))

    return response_success("Skills updated", data={"user_skill_ids": sorted(enabled)})
