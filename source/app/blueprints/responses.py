    ...
    def default(self, obj):
        ...
        if isinstance(obj, uuid.UUID):
            ...
            if obj.__class__ == bytes:
                # Previously pickle.load(obj) with a str(obj) fallback. load()
                # takes a file object, so on bytes it always raised and the
                # fallback always ran -- the branch was inert by accident.
                # Do NOT "correct" this to pickle.loads(): that would
                # deserialise arbitrary bytes while encoding an API response.
                return str(obj)
