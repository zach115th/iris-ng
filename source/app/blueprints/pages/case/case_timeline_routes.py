...
def case_add_event_modal(caseid, url_redir):
    ...
    # iris-next: deep-linked prefill — the Assets/IOC pages' Timeline tabs
    # open this modal with their object preselected. The id is validated
    # against THIS case's own catalog (already in hand), so a foreign or
    # garbage id prefills nothing instead of leaking into the picker.
    assets_prefill = None
    iocs_prefill = None
    # NB the _for_tm helpers return DICTS (with a '0' sentinel first row),
    # not ORM rows — attribute access here 500s.
    preset_asset = request.args.get('preset_asset', type=int)
    if preset_asset is not None and any(
            a.get('asset_id') == preset_asset for a in assets):
        assets_prefill = [preset_asset]
    preset_ioc = request.args.get('preset_ioc', type=int)
    if preset_ioc is not None and any(
            i.get('ioc_id') == preset_ioc for i in iocs):
        iocs_prefill = [preset_ioc]

    return render_template("modal_add_case_event.html", form=form, event=event,
                           verdict_choices=verdict_choices(), default_verdict=DEFAULT_VERDICT,
                           tags=_EVENT_TAGS, assets=assets, iocs=iocs, assets_prefill=assets_prefill,
                           iocs_prefill=iocs_prefill, category=def_cat,
