...
from app.datamgmt.case.case_assets_db import get_linked_evidence_id_from_asset
...
def add_asset_modal(caseid, url_redir):
    ...
    ioc = get_iocs(caseid)
    # Evidence files registered on the case — feeds the "Related Evidence" picker
    evidence = get_rfiles(caseid)
    attributes = get_default_custom_attributes('asset')

    return render_template("modal_add_case_multi_asset.html", form=form, asset=None, ioc=ioc,
...
def asset_view_modal(cur_id, caseid, url_redir):
    ...
    # Evidence files registered on the case + the ones already linked to this asset
    case_evidence = get_rfiles(caseid)
    asset_evidence = get_linked_evidence_id_from_asset(cur_id)
    evidence_prefill = [row for row in asset_evidence]
    ...
    return render_template("modal_add_case_asset.html", form=form, asset=asset, map={}, ioc=case_iocs,
                           ioc_prefill=ioc_prefill, evidence=case_evidence, evidence_prefill=evidence_prefill,
                           attributes=asset.custom_attributes, comments_map=comments_map)
