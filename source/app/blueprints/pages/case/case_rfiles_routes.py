...
from app.datamgmt.case.case_assets_db import get_linked_assets_from_evidence
...
def case_edit_rfile_modal(cur_id, caseid, url_redir):
    ...
    # Inverse view of the asset-modal's "Related Evidence" picker — when an
    # asset is linked to this evidence, surface a chip back to that asset so
    # the analyst can navigate either direction without having to open every
    # asset in turn.
    linked_assets = get_linked_assets_from_evidence(cur_id)

    # iris-next: case users seed the "Created by" combo (IRIS-user typeahead +
    # free text for external custodians); drives seed the "On physical drive"
    # picker (Inventory tab linkage).
    ...
    return render_template("modal_add_case_rfile.html", rfile=crf, attributes=crf.custom_attributes,
                           comments_map=comments_map, linked_assets=linked_assets, users=users, drives=drives)
...
def case_add_rfile_modal(caseid, url_redir):
    ...
    return render_template("modal_add_case_rfile.html", rfile=None,
                           attributes=get_default_custom_attributes('evidence'), users=users, drives=drives)
