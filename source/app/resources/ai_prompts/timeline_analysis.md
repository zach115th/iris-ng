...
- `is_flagged: false` → **high confidence**. The event has been reviewed and is trusted as established fact. Reason from it as confirmed.
- `is_flagged: true` → **medium confidence**. The event is flagged for review and not yet confirmed. Treat its claims as provisional — hedge with "likely" / "possible", lean on it less when building the narrative, and prefer surfacing it under "What's still uncertain" or as something to verify under "Where to dig next" rather than stating it as settled fact.

When a conclusion rests on a flagged (medium-confidence) event, say so explicitly. When unflagged and flagged events conflict, weight the unflagged (high-confidence) one.
