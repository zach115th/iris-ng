...
def create_asset(asset, caseid, user_id):
    ...
    # iris-ng v2 (Phase 4): org-wide registry sync — same single-funnel
    # reasoning. Runs on its own engine connection and never raises.
    from app.datamgmt.manage.customer_assets_db import sync_case_asset
    sync_case_asset(asset)
...
def get_assets_evidence_links(caseid):
    """Bulk variant of get_linked_evidence_finfo_from_asset for the asset
    list payload — one query for the whole case, mirroring
    get_assets_ioc_links (the per-row loop is the N+1 shape the IOC list
    fix removed)."""
...
def delete_evidence_asset_link(asset_id):
    EvidenceAssetLink.query.filter(
        EvidenceAssetLink.asset_id == asset_id
    ).delete()


def get_linked_evidence_id_from_asset(asset_id):
    rows = EvidenceAssetLink.query.with_entities(
...
def get_linked_evidence_finfo_from_asset(asset_id):
    rows = EvidenceAssetLink.query.with_entities(
        CaseReceivedFile.id.label('evidence_id'),
        CaseReceivedFile.filename,
...
def get_linked_assets_from_evidence(evidence_id):
    """Inverse of get_linked_evidence_finfo_from_asset — returns the assets
    that have been associated with a given evidence file. Used by the
    evidence-edit modal to render `Linked Assets` chips."""
    rows = EvidenceAssetLink.query.with_entities(
        CaseAssets.asset_id,
        CaseAssets.asset_name,
        CaseAssets.asset_uuid,
        ...
        EvidenceAssetLink.evidence_id == evidence_id,
        EvidenceAssetLink.asset_id == CaseAssets.asset_id,
        CaseAssets.asset_type_id == AssetsType.asset_id
    )).order_by(CaseAssets.asset_name.asc()).all()

    return rows


def set_evidence_links(evidence_list, asset_id):
    """Replace the evidence-link set for an asset.

    Mirrors `set_ioc_links`: delete-all then bulk-insert. UNIQUE on
    (asset_id, evidence_id) makes the table idempotent if a stray duplicate
    sneaks past concurrent writers.
    """
    if evidence_list is None:
        return False, "Empty evidence list"

    delete_evidence_asset_link(asset_id)

    for evidence_id in evidence_list:
        eal = EvidenceAssetLink()
        eal.asset_id = asset_id
        eal.evidence_id = evidence_id
        db.session.add(eal)
