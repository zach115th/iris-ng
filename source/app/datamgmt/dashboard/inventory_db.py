...
def compute_capacity_planning():
    ...
    available_count = EvidenceDrive.query.filter(EvidenceDrive.status == 'available').count()

    # Wipe-eligible: in_use drives past or within 30 days of the retention threshold.
    # When data retention period is over the drive goes back into rotation (wiped),
    # so these are the drives that will free up in the next ~30 days.
    ...
    effective_supply = available_count + wipe_eligible_count
