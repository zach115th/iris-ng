...
def delete_client(client_id: int) -> None:
    ...
    try:
        # iris-ng fix (upstream defect, byte-identical in v2.5.0-beta.1):
        # /manage/customers/add grants the CREATOR a user_client access row,
        # and this function's blanket except turned that row's FK violation
        # into "referenced customer" — so no customer created through the UI
        # could ever be deleted through the UI. Access grants are ACL
        # metadata, not business references; clear them with the customer.
        # Real references (cases, alerts, contacts, assets) still block.
        UserClient.query.filter(UserClient.client_id == client_id).delete()
