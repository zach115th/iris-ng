...
def list_cases_dict(user_id):
    ...
    res = UserCaseEffectiveAccess.query.with_entities(
        ...
        # iris-next: additive — the v3 list's per-row Close action carries
        # the #84 reviewer-before-close UI gate, which needs to know
        # whether a reviewer is assigned without opening the modal.
        Cases.reviewer_id,
...
def delete_case(case_id):
    ...
    # Vanilla DFIR-IRIS imports keep the legacy `ioc_link` M2M table around:
    # upstream migration 3715d4fac4de migrated its rows into `ioc.case_id`
    # but left `op.drop_table('ioc_link')` commented out, so the table and
    # its FK to ioc.ioc_id stay intact. On such DBs the Ioc.query.delete()
    # below fails with a foreign-key violation and the whole transaction
    # rolls back, leaving the case undeleted. Drop matching rows first.
    if db.session.execute(text("SELECT to_regclass('public.ioc_link')")).scalar():
        db.session.execute(
            text('DELETE FROM ioc_link WHERE case_id = :cid '
                 'OR ioc_id IN (SELECT ioc_id FROM ioc WHERE case_id = :cid)'),
            {'cid': case_id})

    # Delete all IOC-dependent tables BEFORE deleting IOCs (foreign key constraints)
    # iris-ng MISP sync module creates misp_attribute_link and misp_event_link:
    # misp_attribute_link has FK to ioc.ioc_id, so delete them before IOCs
    if db.session.execute(text("SELECT to_regclass('public.misp_attribute_link')")).scalar():
        db.session.execute(
            text('DELETE FROM misp_attribute_link '
                 'WHERE ioc_id IN (SELECT ioc_id FROM ioc WHERE case_id = :cid)'),
            {'cid': case_id})

    if db.session.execute(text("SELECT to_regclass('public.misp_event_link')")).scalar():
        db.session.execute(
            text('DELETE FROM misp_event_link WHERE case_id = :cid'),
            {'cid': case_id})

    # Delete case_events_ioc (references IOC) before deleting IOCs
    CaseEventsIoc.query.filter(CaseEventsIoc.case_id == case_id).delete()

    # Delete ioc_note_link (iris-ng feature: IOC source provenance)
    if db.session.execute(text("SELECT to_regclass('public.ioc_note_link')")).scalar():
        db.session.execute(
            text('DELETE FROM ioc_note_link '
                 'WHERE ioc_id IN (SELECT ioc_id FROM ioc WHERE case_id = :cid)'),
            {'cid': case_id})

    # Delete IocAssetLink (references IOC)
    IocAssetLink.query.filter(
        IocAssetLink.ioc_id.in_(
            db.session.query(Ioc.ioc_id).filter(Ioc.case_id == case_id)
        )
    ).delete()
    ...
    # iris-ng fix (2026-09-01): alert escalation/merge re-anchors the ALERT'S
    # OWN Ioc rows into the case (add_ioc sets case_id on the same row), so a
    # blind delete here violates alert_iocs_association's FK and the whole
    # delete rolls back — any case escalated from an alert with imported
    # IOCs was undeletable while the alert lived. Mirror the asset treatment
    # below: DETACH rows still referenced by alerts (back to their alert-only
    # state), delete only the rest.
    Ioc.query.filter(
        Ioc.case_id == case_id,
        Ioc.ioc_id.in_(db.session.query(alert_iocs_association.c.ioc_id))
    ).update({Ioc.case_id: None}, synchronize_session=False)
