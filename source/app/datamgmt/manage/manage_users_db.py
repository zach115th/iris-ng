...
def get_user_details(user_id, include_api_key=False):
    ...
    # iris-ng: analyst billing rate for case-cost estimation (None = unpriced)
    row['user_hourly_rate'] = float(user.hourly_rate) if user.hourly_rate is not None else None
    # iris-ng: enabled cyber-security skill ids (for the Skills tab)
    row['user_skill_ids'] = sorted(get_user_skill_ids(user_id))
...
def update_user_hourly_rate(user: User, hourly_rate):
    """iris-ng: set/clear a user's billing rate (currency-units per hour) for
    case-cost estimation. `hourly_rate` is a non-negative number, or None to
    clear (unprice the user)."""
...
def get_skills_catalog():
    """iris-ng: the active skill catalog, ordered by category then name.
    Returns list of dicts for templates / API."""
    rows = (
        Skill.query.filter(Skill.is_active == True)  # noqa: E712 (SQL boolean)
        .order_by(Skill.skill_category.asc(), Skill.skill_name.asc())
        .all()
    )
    return [
        {
...
def get_user_skill_ids(user_id):
    """iris-ng: set of skill ids enabled for a user."""
    rows = UserSkill.query.filter(UserSkill.user_id == user_id).with_entities(UserSkill.skill_id).all()
    return {r.skill_id for r in rows}


def set_user_skills(user_id, skill_ids):
    """iris-ng: replace the user's enabled-skill set with `skill_ids`.

    Delete-all + bulk-insert (same shape as set_ioc_links / set_evidence_links).
    Only skill ids that exist in the catalog are inserted — unknown ids are
    silently dropped. Returns the resulting set of enabled ids.
    """
    wanted = {int(s) for s in (skill_ids or [])}
    if wanted:
        valid = {
            r.id for r in Skill.query.filter(Skill.id.in_(wanted)).with_entities(Skill.id).all()
        }
        wanted = wanted & valid

    UserSkill.query.filter(UserSkill.user_id == user_id).delete()
    for sid in wanted:
