...
def case_template_to_dict(case_template: CaseTemplate) -> dict:
    """Full editable JSON form of a template — the ONE builder shared by the
    legacy modal route and the v2 read endpoint so the two cannot drift.

    Args:
        case_template (CaseTemplate): case template

    Returns:
        dict: the template's editable fields
    """
    return {
