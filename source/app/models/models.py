...
class CaseAiArtifact(db.Model):
    ...
    # iris-next: analyst manual override. `content` always keeps the ORIGINAL
    # model output so "View AI original" / "Revert to AI" stay available;
    # `edited_content` is the analyst's corrected text and, when non-NULL, is
    # what every read surface displays. Regeneration is guarded at the endpoint
    # layer — a new generation inserts a NEW row, so an unguarded regen would
    # silently orphan the edit.
    edited_content = Column(Text, nullable=True)
    edited_by_id = Column(ForeignKey('user.id'), nullable=True)
