...
class IrisMISPSyncInterface(IrisModuleInterface):
    ...
    def hooks_handler(self, hook_name: str, hook_ui_name: str, data: Any):
        ...
        # NOTE on fork-safety: celery prefork workers fork() children that
        # inherit the parent's live psycopg2 connection, which is NOT fork-safe
        # (first commit crashes with `PGRES_TUPLES_OK ... no message from the
        # libpq`). That is now handled CENTRALLY in
        # app/iris_engine/tasker/tasks.py::on_task_init (a `task_prerun` signal
        # that does `db.session.remove()` + `db.engine.dispose()`), which fires
        # BEFORE task_hook_wrapper merges the dispatched ORM objects — so they
        # stay session-bound when handed to us here.
        #
        # This module used to call `db.session.remove()` itself at this point,
        # but that ran AFTER task_hook_wrapper had already merged `case`/`ioc`
        # into the session and passed them in `data`; removing the session
        # detached those instances and the first lazy attribute access (e.g.
        # `case.case_id`) raised `DetachedInstanceError`. The central handler
        # makes a per-module shim both redundant and harmful — do NOT
        # reintroduce a `db.session.remove()` here.
