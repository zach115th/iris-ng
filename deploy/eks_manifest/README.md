...
> ## ⚠️ Unmaintained example — the Helm chart is the supported path
>
> This directory is inherited from upstream and is **not** covered by IRIS-NG's
> release testing. For Kubernetes use `deploy/kubernetes/charts/`, which is
> deploy-tested each release and whose image tags follow the chart `appVersion`.
>
> ### Every secret here is a placeholder
>
> The `secrets.yml` files previously shipped **working credentials** — a Postgres
> password of `admin` and the well-known `IRIS_SECRET_KEY` — and this guide sent
> you straight to `make create` without mentioning them. They are now
> `__MUST_BE_CHANGED__` and **must be replaced before deploying**:
>
> ```bash
> # a fresh secret key / salt
> python -c "import secrets; print(secrets.token_urlsafe(48))"
>
> # values in a Secret are base64, not encrypted
> printf '%s' 'your-value' | base64
> ```
>
> `IRIS_SECRET_KEY` signs session cookies. Deploying with a publicly known value
> lets anyone forge a session for any user, including the administrator.
> Kubernetes Secrets are also not encrypted at rest by default — prefer a real
> secrets manager over committing a filled-in copy of these files.
